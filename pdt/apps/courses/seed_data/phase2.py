...
cases.</p>"""
            ...
            "questions": [
                q("O que é um hypervisor tipo 1?",
                  ...
                  ["Roda dentro de um sistema operacional host.",
                   "É um container isolado por namespaces.",
                   "Substitui o BIOS da placa-mãe."],
                  "Tipo 1 = bare-metal (KVM, ESXi, Hyper-V). Tipo 2 = roda como app dentro de "
                  "outro SO (VirtualBox). Container não é hypervisor: isola processos que "
                  "dividem o kernel do host. E nenhum hypervisor substitui o firmware da placa.",
                  statement_en="What is a type 1 hypervisor?",
                  correct_en="Runs directly on the hardware, with no host OS.",
                  wrong_en=["Runs inside a host operating system.",
                            "It is a container isolated by namespaces.",
                            "It replaces the motherboard's BIOS."],
                  explanation_en="Type 1 = bare-metal (KVM, ESXi, Hyper-V). Type 2 runs as an app "
                  "inside another OS (VirtualBox). A container is not a hypervisor: it isolates "
                  "processes sharing the host kernel. And no hypervisor replaces board firmware."),
                q("Cloud difere de virtualização porque:",
                  "Adiciona APIs, self-service e billing por uso.",
                  ["Não usa hardware físico algum.",
                   "Não oferece suporte a Linux no catálogo.",
                   "Não tem rede entre os servidores do datacenter."],
                  "Virtualização é fundação técnica; cloud junta automação, multi-tenancy e billing. "
                  "A nuvem continua rodando sobre hardware físico real, com rede entre os "
                  "servidores e um catálogo Linux amplo.",
                  statement_en="Cloud differs from virtualization because it:",
                  correct_en="Adds APIs, self-service, and usage-based billing.",
                  wrong_en=["Uses no physical hardware at all.",
                            "Offers no Linux support in its catalog.",
                            "Has no network between the datacenter servers."],
                  explanation_en="Virtualization is the technical foundation; cloud adds automation, "
                  "multi-tenancy, and billing. The cloud still runs on real physical hardware, with "
                  "a network between servers and a broad Linux catalog."),
                q("VM e container compartilham:",
                  "O hardware via virtualização (mesmo host físico).",
                  ["O mesmo hypervisor rodando abaixo de ambos.",
                   "O mesmo kernel do sistema operacional hospedeiro.",
                   "O mesmo endereço IP público na interface de rede."],
                  "Containers compartilham kernel; VMs têm kernel próprio. Ambos podem rodar no mesmo host. "
                  "O container não passa por hypervisor, e cada VM ou container recebe seu próprio endereço.",
                  statement_en="A VM and a container share:",
                  correct_en="The hardware via virtualization (same physical host).",
                  wrong_en=["The same hypervisor running underneath both of them.",
                            "The same kernel of the underlying host operating system.",
                            "The same public IP address on the network interface."],
                  explanation_en="Containers share the kernel; VMs have their own kernel. Both can "
                  "run on the same host. A container does not go through a hypervisor, and each one "
                  "gets its own address."),
                q("Vantagem de cloud para startups:",
                  "Capex baixo e elasticidade.",
                  ["Hardware proprietário incluído.",
                   "Dispensa monitoramento.",
                   "Latência de rede zero."],
                  "Pay-as-you-go evita compra de hardware para pico que pode nem existir. "
                  "Elasticidade não elimina latência de rede nem a necessidade de monitorar, e o "
                  "hardware continua sendo do provedor.",
                  statement_en="Advantage of cloud for startups:",
                  correct_en="Low capex and elasticity.",
                  wrong_en=["Proprietary hardware included.",
                            "No monitoring needed.",
                            "Zero network latency."],
                  explanation_en="Pay-as-you-go avoids buying hardware for a peak that might never "
                  "even happen. Elasticity removes neither network latency nor the need to monitor, "
                  "and the hardware still belongs to the provider."),
                q("Tipo de cloud onde recursos são exclusivos da empresa:",
                  ...
                  "Public é infraestrutura compartilhada entre clientes, hybrid combina as duas e "
                  "edge aproxima o processamento do usuário final.",
                  statement_en="Type of cloud where resources are exclusive to the company:",
                  correct_en="Private cloud.",
                  wrong_en=["Public cloud.",
                            "Hybrid cloud.",
                            "Edge computing."],
                  explanation_en="Can be on-premise (your own datacenter) or a dedicated VPC in a "
                  "public provider. Public is infrastructure shared among customers, hybrid combines "
                  "both, and edge moves processing closer to the end user."),
                q("Qual hypervisor é open source e parte do kernel Linux?",
                  ...
                  "completam o ecossistema. Xen é open source, mas um hypervisor separado, fora "
                  "da árvore do kernel; ESXi (VMware) e Hyper-V (Microsoft) são proprietários.",
                  statement_en="Which hypervisor is open source and part of the Linux kernel?",
                  correct_en="KVM",
                  wrong_en=["Xen", "ESXi", "Hyper-V"],
                  explanation_en="KVM ('Kernel-based Virtual Machine') is a kernel module; libvirt "
                  "and QEMU complete the ecosystem. Xen is open source but a separate hypervisor, "
                  "outside the kernel tree; ESXi (VMware) and Hyper-V (Microsoft) are proprietary."),
                q("'Pay-as-you-go' refere-se a:",
                  "Cobrar pelo uso real, sem compromisso de longo prazo.",
                  ["Pagar o valor inteiro antes de usar o serviço.",
                   "Usar o serviço de graça enquanto ele estiver ativo.",
                  ...
                  "Pagamento adiantado existe, mas é o modelo de Reserved Instance, não pay-as-you-go.",
                  statement_en="'Pay-as-you-go' refers to:",
                  correct_en="Charging for actual usage, with no long-term commitment.",
                  wrong_en=["Paying the full amount upfront, before using the service.",
                            "Using the service for free while it stays active.",
                            "Paying only if the project succeeds by year end."],
                  explanation_en="In many services (S3, Lambda, Run) you pay per request/byte. On "
                  "VMs, per hour/second. Upfront payment exists, but that is the Reserved Instance "
                  "model, not pay-as-you-go."),
                q("Multi-tenancy implica:",
                  "Múltiplos clientes compartilhando infra com isolamento.",
                  ["Banco único, sem separação lógica entre clientes.",
                   "Ausência de segregação entre cargas de trabalho.",
                   "Um cliente por servidor físico, sem compartilhamento de hardware."],
                  "Isolamento via namespaces, IAM, redes virtuais. Provedor garante que tenant A não vê tenant B. "
                  "Dedicar um servidor físico por cliente é justamente o modelo single-tenant.",
                  statement_en="Multi-tenancy implies:",
                  correct_en="Multiple customers sharing infrastructure with isolation.",
                  wrong_en=["A single database, with no logical separation between customers.",
                            "Absence of segregation between different workloads.",
                            "One customer per physical server, with no hardware sharing."],
                  explanation_en="Isolation via namespaces, IAM, virtual networks. The provider "
                  "guarantees tenant A never sees tenant B. Dedicating one physical server per "
                  "customer is precisely the single-tenant model."),
                q("Region em cloud é:",
                  "Conjunto geográfico de datacenters (várias AZs).",
                  ["Um endereço IP público de uma instância.",
                   "Uma role de IAM que concede acesso a um recurso.",
                   "Uma máquina física dentro de um datacenter."],
                  "Ex.: us-east-1 tem múltiplas AZs (us-east-1a, 1b, 1c). Latência baixa entre AZs, alta entre regiões. "
                  "Region é escopo geográfico, não um recurso individual nem uma permissão.",
                  statement_en="A region in cloud is:",
                  correct_en="A geographic set of datacenters (multiple AZs).",
                  wrong_en=["A public IP address of one instance.",
                            "An IAM role that grants access to a given resource.",
                            "A physical machine inside a datacenter."],
                  explanation_en="E.g.: us-east-1 has multiple AZs (us-east-1a, 1b, 1c). Latency is "
                  "low between AZs, higher between regions. A region is a geographic scope, not an "
                  "individual resource nor a permission."),
                q("Disponibilidade aumenta com:",
                  "Distribuir cargas entre múltiplas Availability Zones.",
                  ["Reduzir o número de réplicas ativas.",
                   "Fazer backup periódico do banco principal.",
                   "Usar um único disco SSD de alta performance sem redundância."],
                  "AZ é falha unitária, se uma cair, as outras sobrevivem se você arquitetou multi-AZ. "
                  "Backup protege o dado, mas não mantém o serviço no ar; menos réplicas e disco único "
                  "concentram o ponto de falha.",
                  statement_en="Availability increases with:",
                  correct_en="Distributing workloads across multiple Availability Zones.",
                  wrong_en=["Reducing the number of active replicas.",
                            "Doing periodic backups of the main database.",
                            "Using a single high-performance SSD disk with no redundancy."],
                  explanation_en="An AZ is the unit of failure — if one goes down, the others survive "
                  "if you architected for multi-AZ. Backups protect the data but don't keep the "
                  "service up; fewer replicas and a single disk concentrate the failure point."),
            ],
        },
        # =====================================================================
...
"""
            ...
            "questions": [
                ...
                q("O provedor de cloud é responsável por:",
                  ...
                  ["Aplicar patch na aplicação do cliente.",
                   "Configurar a política de IAM da conta do cliente.",
                   "Fazer backup do banco de dados do cliente."],
                  "DA cloud (segurança da infraestrutura) é do provedor; NA cloud (o que você roda nela) é seu. "
                  "Patch de aplicação, política de IAM da conta e backup do dado caem todos do lado do cliente.",
                  statement_en="The cloud provider is responsible for:",
                  correct_en="Security OF the cloud (datacenter, hypervisor).",
                  wrong_en=["Patching the customer's application.",
                            "Configuring the customer account's IAM policy.",
                            "Backing up the customer's database."],
                  explanation_en="Security OF the cloud (infrastructure security) is the "
                  "provider's; security IN the cloud (what you run in it) is yours. Application "
                  "patching, account IAM policy and data backup all fall on the customer's side."),
                q("Em SaaS típico, o que ainda é dever do cliente?",
                  "Identidades, dados e configuração.",
                  ["O hardware físico do datacenter.",
                   "O sistema operacional da VM subjacente.",
                   "O hypervisor que isola cada cliente."],
                  "Mesmo no SaaS, gerência de usuários, MFA e classificação de dados não saem do cliente. "
                  "Hardware, SO da VM e hypervisor ficam inteiramente com o provedor nesse modelo.",
                  statement_en="In typical SaaS, what still falls on the customer?",
                  correct_en="Identities, data, and configuration.",
                  wrong_en=["The datacenter's physical hardware.",
                            "The operating system of the underlying VM.",
                            "The hypervisor that isolates each customer."],
                  explanation_en="Even in SaaS, user management, MFA, and data classification "
                  "never leave the customer's hands. Hardware, VM OS and hypervisor belong "
                  "entirely to the provider in this model."),
                q("Quem responde por uma S3 mal configurado e público?",
                  "O cliente, é configuração dele.",
                  ["A AWS, é bug conhecido do S3.",
                   "O provedor, o bucket nasce protegido.",
                   "Dividida meio a meio entre os dois."],
                  "AWS oferece 'Block Public Access' habilitado por padrão; cliente desligou? Cliente respondeu. "
                  "O bucket realmente nasce fechado — abri-lo é uma ação deliberada de configuração, não uma falha do serviço.",
                  statement_en="Who is on the hook for a misconfigured, public S3 bucket?",
                  correct_en="The customer — it's their configuration.",
                  wrong_en=["AWS — it's a known S3 bug.",
                            "The provider; buckets are born protected.",
                            "Split evenly between the two parties."],
                  explanation_en="AWS ships 'Block Public Access' enabled by default; did the "
                  "customer turn it off? Then the customer answers for it. The bucket really is "
                  "born closed — opening it is a deliberate configuration action, not a service bug."),
                q("Backup de dados em RDS é dever:",
                  "Do cliente, que configura snapshots e retenção.",
                  ["Do provedor, que faz backup automático de fábrica.",
                   "Da equipe de auditoria externa contratada.",
                   "Do RDS, que retém snapshots indefinidamente."],
                ...
                q("Qual fator NÃO faz parte do shared responsibility?",
                  ...
                  ["A rede virtual (VPC).",
                   "Identidade e política de acesso do usuário.",
                   "Dados armazenados e processados nos serviços."],
                  "Pegadinha, todos os outros são partes legítimas do modelo: identidade, dado e "
                  "rede virtual estão explicitamente no lado do cliente.",
                  statement_en="Which factor is NOT part of shared responsibility?",
                  correct_en="The color of the company's logo.",
                  wrong_en=["The virtual network (VPC).",
                            "User identity and access policy.",
                            "Data stored and processed in the services."],
                  explanation_en="Trick question — all the others are legitimate parts of the "
                  "model: identity, data and the virtual network sit explicitly on the customer side."),
                q("Em IaaS, patch do kernel é:",
                  "Responsabilidade do cliente.",
                  ["Tarefa do provedor.",
                   "Automatizado pela cloud por padrão.",
                   "Dispensável em VM de curta duração."],
                  "Use Systems Manager/Update Management para automatizar, mas a responsabilidade é sua. "
                  "O provedor aplica patch no hypervisor, não no kernel do seu guest, e VM efêmera "
                  "também roda kernel vulnerável enquanto estiver de pé.",
                  statement_en="In IaaS, kernel patching is:",
                  correct_en="The customer's responsibility.",
                  wrong_en=["The provider's task.",
                            "Automated by the cloud by default.",
                            "Unnecessary on short-lived VMs."],
                  explanation_en="Use Systems Manager/Update Management to automate it, but the "
                  "responsibility is yours. The provider patches the hypervisor, not your guest "
                  "kernel, and an ephemeral VM runs a vulnerable kernel while it is up too."),
                q("Compliance é responsabilidade:",
                  "Compartilhada, cada parte certifica o que controla.",
                  ["Do auditor externo contratado uma vez por ano pela empresa.",
                   "Do provedor, que certifica o datacenter.",
                   "Do cliente, que prova conformidade sozinho."],
                  "Provedor mostra que o datacenter está conforme; cliente mostra que sua app/processo está conforme. "
                  "O auditor verifica evidência, não produz conformidade.",
                ...
                q("Por que ler documentos do provedor?",
                  ...
                  ["Por exigência legal, sem efeito prático.",
                   "Para virar parceiro comercial certificado do provedor.",
                   "Para reduzir o custo mensal do serviço."],
                  "Surpresas em incidente são caras; ler antes evita 'nossa, achei que vocês cuidassem disso'. "
                  "O documento define fronteira de responsabilidade — não muda preço nem status comercial.",
                  statement_en="Why read the provider's documents?",
                  correct_en="To know the exact boundary and not assume coverage.",
                  wrong_en=["A legal requirement, with no practical effect.",
                            "To become a certified commercial partner of the provider.",
                            "To reduce the service's monthly cost."],
                  explanation_en="Surprises during an incident are expensive; reading beforehand "
                  "avoids 'oh, I thought you handled that'. The document defines the responsibility "
                  "boundary — it changes neither price nor commercial status."),
                q("Configuração errada em segurança em cloud é a causa:",
                  "Mais comum de incidentes em cloud pública.",
                  ["Mais rara que ataque à infraestrutura do provedor.",
                   "Atribuída ao provedor pelo contrato.",
                   "Restrita à área de billing da conta."],
                  "Confirmado por DBIR, CSA, Gartner, AWS Well-Architected, misconfiguration domina o ranking. "
                  "Invadir o datacenter do provedor é raríssimo perto de um bucket aberto por engano.",
                  statement_en="Security misconfiguration in cloud is the:",
                  correct_en="Most common cause of incidents in public cloud.",
                  wrong_en=["Rarer cause than an attack on the provider's infrastructure.",
                            "Cause attributed to the provider by contract.",
                            "Cause limited to the account's billing area."],
                  explanation_en="Confirmed by DBIR, CSA, Gartner, AWS Well-Architected — "
                  "misconfiguration dominates the ranking. Breaching the provider's datacenter is "
                  "vanishingly rare next to a bucket opened by mistake."),
            ],
        },
        # =====================================================================
        # 2.3 IAM
...
as just another ordinary credential on the internal network.</p>"""
            ...
            "questions": [
                q("Vantagem de role sobre chave estática:",
                  ...
                  ["Custa menos que uma chave estática.",
                   "Cota maior de requisições por segundo.",
                   "Handshake mais rápido na autenticação de cada chamada de API."],
                  "Chave temporária expira sozinha; chave estática vaza e fica em open-source forever. "
                  "Role não é mais barata, não muda cota e não acelera o handshake: a diferença está "
                  "no ciclo de vida da credencial.",
                  statement_en="Advantage of a role over a static key:",
                  correct_en="Temporary credentials, with no persistent storage.",
                  wrong_en=["It costs less than a static key.",
                            "A higher per-second request quota.",
                            "A faster handshake when authenticating each API call."],
                  explanation_en="A temporary credential expires on its own; a static key leaks and stays valid in open-source forever. "
                  "A role is not cheaper, does not change quota and does not speed up the handshake: the difference is the credential's lifecycle."),
                q("MFA serve para:",
                  "Adicionar segundo fator (algo que você tem) à autenticação.",
                  ["Substituir a senha por completo.",
                   "Aumentar a cota de requisições do usuário.",
                   "Gerar um token de DNS usado para provar a posse de um domínio."],
                  "Reduz drasticamente o risco de credential stuffing, mesmo se a senha vaza, MFA segura. "
                  "O fator adicional soma-se à senha em vez de substituí-la, e não tem relação com cota nem com validação de domínio.",
                  statement_en="MFA exists to:",
                  correct_en="Add a second factor (something you have) to authentication.",
                  wrong_en=["Replace the password entirely.",
                            "Increase that user's request quota.",
                            "Generate a DNS token used to prove ownership of a domain."],
                  ...
                  "The extra factor adds to the password instead of replacing it, and has nothing to do with quotas or domain validation."),
                q("Policy IAM é avaliada como:",
                  "Combinação de Allow/Deny, Deny sempre vence em conflito.",
                  ["A última regra escrita na política vence.",
                   "Considera o Allow mais recente da política.",
                   "Escolhida aleatoriamente entre as regras aplicáveis ao caso."],
                  "Sem Allow explícito, default é negar. Deny sempre wins, mesmo se houver Allow em outra policy. "
                  "A avaliação é determinística e não depende da ordem nem da data em que a regra foi escrita.",
                  statement_en="An IAM policy is evaluated as:",
                  correct_en="A combination of Allow/Deny, where an explicit Deny always wins.",
                  wrong_en=["The last rule written in the policy wins.",
                            "It considers the most recent Allow in the policy.",
                            "Chosen at random among the rules that apply to the case."],
                  explanation_en="With no explicit Allow, the default is to deny. Deny always wins, even if an Allow exists in another policy. "
                  "Evaluation is deterministic and does not depend on rule order or on when the rule was written."),
                q("Para devs sem precisar de console, prefira:",
                  "Roles e federação (IAM Identity Center / SSO).",
                  ["Usuário IAM com chave de longa duração.",
                   "Compartilhar uma credencial entre os devs do time.",
                   "Senha enviada por SMS a cada login."],
                  "Acesso temporário emitido por SSO; nada de chave longa flutuando em ~/.aws/credentials. "
                  "Credencial compartilhada destrói a rastreabilidade, e SMS é fator fraco, sujeito a SIM swap.",
                  statement_en="For devs who don't need console access, prefer:",
                  correct_en="Roles and federation (IAM Identity Center / SSO).",
                  wrong_en=["An IAM user with a long-lived key.",
                            "Sharing one credential among the team's devs.",
                            "A password sent by SMS on every login."],
                  explanation_en="Temporary access issued by SSO; no long-lived key floating around in ~/.aws/credentials. "
                  "A shared credential destroys traceability, and SMS is a weak factor, exposed to SIM swap."),
                q("SCP em AWS Organizations serve para:",
                  "Criar guardrails que limitam o que contas filhas podem fazer.",
                  ["Substituir a VPC da conta por uma rede gerenciada.",
                   "Acelerar o tempo de deploy de uma aplicação na conta.",
                   "Aumentar o limite de gasto na fatura consolidada da organização."],
                  "SCP é teto: mesmo Admin de uma sub-conta não escapa. Útil para impor compliance. "
                  "SCP é controle de permissão, não toca em rede, em pipeline de deploy nem em orçamento.",
                  statement_en="An SCP in AWS Organizations is used to:",
                  correct_en="Create guardrails that limit what child accounts can do.",
                  wrong_en=["Replace the account's VPC with a managed network.",
                            "Speed up deploy time for an application in the account.",
                            "Raise the spending limit on the organization's consolidated invoice."],
                  explanation_en="An SCP is a ceiling: even a sub-account's Admin can't escape it. Useful for enforcing compliance. "
                  "An SCP is a permission control; it touches neither networking, nor deploy pipelines, nor budgets."),
                q("Por que rotacionar access keys?",
                  "Limita o impacto se uma chave vazar.",
                  ["Reduz o custo mensal da conta.",
                   "É requisito do protocolo HTTPS na API.",
                   "Aumenta a velocidade da chamada de API."],
                  "Janela de exploração de uma chave vazada é o tempo entre vazamento e próxima rotação. "
                  "Rotação é higiene de credencial: não muda preço, protocolo nem desempenho da chamada.",
                  statement_en="Why rotate access keys?",
                  correct_en="Limits the impact if a key leaks.",
                  wrong_en=["Reduces the account's monthly cost.",
                            "It's an HTTPS protocol requirement for the API.",
                            "Increases the speed of the API call."],
                  explanation_en="The exploitation window of a leaked key is the time between the leak and the next rotation. "
                  "Rotation is credential hygiene: it changes neither price, protocol, nor call performance."),
                q("Como aplicar PoLP em IAM?",
                  ...
                  "Abrir tudo 'temporariamente' costuma virar permanente, porque ninguém sabe depois o que dá para tirar.",
                  ...
                  "Opening everything 'temporarily' tends to become permanent, because nobody later knows what is safe to remove."),
                q("Qual recurso registra quem fez o quê na AWS?",
                  ...
                  "Athena consulta dado já armazenado, Flow Logs registram tráfego de rede e o log de "
                  "acesso do S3 cobre só aquele bucket.",
                  statement_en="Which resource logs who did what in AWS?",
                  correct_en="AWS CloudTrail.",
                  wrong_en=["Amazon Athena.",
                            "VPC Flow Logs.",
                            "S3 access logs."],
                  explanation_en="CloudTrail logs API calls. Enable an org-wide trail and send it to a protected bucket. "
                  "Athena queries data already stored, Flow Logs record network traffic, and S3 access logging covers only that bucket."),
                q("Diferença entre user e role:",
                  "User tem credenciais permanentes; role é assumida temporariamente.",
                  ["User serve a humano; role, a máquina.",
                   "Role é um tipo de política anexada ao usuário.",
                   "Assumir uma role custa mais caro do que manter um usuário fixo na conta."],
                  "Humano pode assumir role via SSO; máquina via STS AssumeRole. Ambos podem. "
                  "Role não é policy (ela tem policies anexadas) e assumir role não gera custo extra.",
                  statement_en="Difference between a user and a role:",
                  correct_en="A user has permanent credentials; a role is assumed temporarily.",
                  wrong_en=["A user is for humans; a role is for machines.",
                            "A role is a kind of policy attached to the user.",
                            "Assuming a role costs more than keeping a fixed user in the account."],
                  explanation_en="A human can assume a role via SSO; a machine via STS AssumeRole. Both are possible. "
                  "A role is not a policy (it has policies attached), and assuming a role costs nothing extra."),
                q("Recomendação para conta root:",
                  "Não usar para tarefas diárias e ativar MFA forte.",
                  ["Desativar o MFA para agilizar o acesso.",
                   "Compartilhar a senha com o time de operação.",
                   "Usar a root no dia a dia, com senha longa e rotacionada."],
                  "Use root só para tarefas que exigem (alterar conta, fechar). MFA hardware ideal. "
                  "Senha forte não compensa o uso rotineiro: a root ignora SCP e boa parte dos limites de IAM.",
                  statement_en="Recommendation for the root account:",
                  correct_en="Don't use it for daily tasks and enable strong MFA.",
                  wrong_en=["Turn off MFA to speed up access.",
                            "Share the password with the operations team.",
                            "Use root day to day, with a long rotated password."],
                  explanation_en="Use root only for tasks that require it (changing the account, closing it). A hardware MFA key is ideal. "
                  "A strong password does not offset routine use: root bypasses SCPs and much of IAM's guardrails."),
            ],
        },
        # =====================================================================
        # 2.4 VPC & Subnets
            ...
            "questions": [
                q("Subnet privada NÃO tem:",
                  ...
                  ["Tabela de rota associada.",
                   "Endereço IP em cada interface.",
                   "Security Group na instância."],
                  "Sem rota para IGW, instâncias não recebem tráfego direto da internet. "
                  "Toda subnet, pública ou privada, tem route table, IPs nas interfaces e SGs; o "
                  "que falta na privada é a rota 0.0.0.0/0 apontando para o IGW.",
                  statement_en="A private subnet does NOT have:",
                  correct_en="A route to an Internet Gateway.",
                  wrong_en=["An associated route table.",
                            "An IP address on each interface.",
                            "A Security Group on the instance."],
                  explanation_en="Without a route to an IGW, instances don't receive traffic directly from the internet. "
                  "Every subnet, public or private, has a route table, interface IPs and SGs; what the private one lacks is the 0.0.0.0/0 route to the IGW."),
                q("Para outbound de subnet privada:",
                  ...
                  "O IGW serve subnet pública, Direct Connect liga ao datacenter próprio e peering liga VPCs — "
                  "nenhum dos três dá saída para a internet a partir de uma subnet privada.",
                  statement_en="For outbound from a private subnet:",
                  correct_en="Use a NAT Gateway.",
                  wrong_en=["Use the IGW directly.",
                            "Use Direct Connect.",
                            "Use VPC Peering."],
                  explanation_en="A NAT Gateway allows egress without exposing the instance. Expensive: consider a "
                  "VPC Endpoint when the destination is AWS. The IGW serves public subnets, Direct Connect links to your own "
                  "datacenter and peering links VPCs — none of the three provides internet egress from a private subnet."),
                ...
                q("VPC Peering serve para:",
                  ...
                  ["Conectar containers no mesmo host.",
                   "Substituir a resolução de nomes do DNS.",
                   "Cobrar valor adicional na fatura da conta."],
                  "Peering é 1:1. Para muitas VPCs, use Transit Gateway (hub-and-spoke). "
                  "Peering é conectividade de rede entre VPCs: não substitui DNS, não conecta "
                  "containers locais e não é um item de cobrança à parte.",
                  statement_en="VPC Peering is used to:",
                  correct_en="Connect two VPCs with non-overlapping IPs.",
                  wrong_en=["Connect containers on the same host.",
                            "Replace name resolution done by DNS.",
                            "Add an extra charge on the account's bill."],
                  explanation_en="Peering is 1:1. For many VPCs, use Transit Gateway (hub-and-spoke). "
                  "Peering is network connectivity between VPCs: it does not replace DNS, connect local containers, or act as a billing line item."),
                q("Por que múltiplas AZs?",
                  "Resiliência a falhas de datacenter inteiro.",
                  ["Reduzir o custo mensal da rede.",
                   "Aumentar a latência entre instâncias.",
                   "Concentrar as réplicas num único datacenter."],
                  "AZ é falha unitária; deploy multi-AZ é o mínimo para serviços de produção. "
                  "Espalhar entre AZs custa tráfego cross-AZ e concentrar réplicas devolve o ponto único de falha.",
                  statement_en="Why multiple AZs?",
                  correct_en="Resilience to an entire datacenter failing.",
                  wrong_en=["Reduce the monthly network cost.",
                            "Increase latency between instances.",
                            "Concentrate replicas in a single datacenter."],
                  explanation_en="An AZ is the unit of failure; multi-AZ deploy is the minimum for production services. "
                  "Spreading across AZs costs cross-AZ traffic, and concentrating replicas brings back the single point of failure."),
                q("Route table define:",
                  "Para onde pacotes de uma subnet vão.",
                  ["A versão de TLS aceita na conexão.",
                   "Qual permissão de IAM o usuário tem.",
                   "O tamanho máximo do pacote (MTU) na interface."],
                  "Cada destino (CIDR) tem next-hop (IGW, NAT, peering, gateway endpoint, etc.). "
                  "TLS é camada de aplicação, IAM é autorização e MTU é atributo da interface — nada disso vive na route table.",
                  statement_en="A route table defines:",
                  correct_en="Where packets from a subnet go.",
                  wrong_en=["Which TLS version the connection accepts.",
                            "Which IAM permission the user has.",
                            "The maximum packet size (MTU) on the interface."],
                  explanation_en="Each destination (CIDR) has a next-hop (IGW, NAT, peering, gateway endpoint, etc.). "
                  "TLS is application layer, IAM is authorization and MTU is an interface attribute — none of it lives in the route table."),
                q("Endpoint VPC para S3 reduz:",
                  "Tráfego que sai pela internet, vai pela rede da AWS.",
                  ["Latência para destinos fora da AWS.",
                   "A necessidade de configurar IAM no bucket.",
                   "O custo cobrado pelo armazenamento de dados no S3 Standard."],
                  "Custos de NAT despencam e exposição também. Use Gateway Endpoint para S3/DynamoDB. "
                  "O endpoint muda o caminho do tráfego: não altera preço de armazenamento, não dispensa IAM "
                  "e não ajuda destinos fora da AWS.",
                  ...
                  wrong_en=["Latency to destinations outside AWS.",
                            "The need to configure IAM on the bucket.",
                            "The cost charged for storing data in S3 Standard."],
                  explanation_en="NAT costs drop sharply, and exposure does too. Use a Gateway Endpoint for S3/DynamoDB. "
                  "The endpoint changes the traffic path: it does not change storage pricing, does not remove the need for IAM, and does not help destinations outside AWS."),
                q("Internet Gateway é:",
                  "Recurso que permite conectividade bidirecional pública.",
                  ["Proxy reverso que intermedeia cliente e servidor de origem.",
                   "Firewall que filtra pacotes na instância.",
                   "Recurso exclusivo para tráfego IPv6 da VPC."],
                  "IGW associado à VPC + rota 0.0.0.0/0 → IGW na route table = subnet pública. "
                  "O IGW não inspeciona nem filtra pacote (isso é SG/NACL) e atende IPv4 e IPv6.",
                  statement_en="An Internet Gateway is:",
                  correct_en="A resource that enables bidirectional public connectivity.",
                  wrong_en=["A reverse proxy that mediates client and origin server.",
                            "A firewall that filters packets at the instance.",
                            "A resource exclusive to the VPC's IPv6 traffic."],
                  explanation_en="IGW attached to the VPC + route 0.0.0.0/0 → IGW in the route table = public subnet. "
                  "The IGW neither inspects nor filters packets (that's SG/NACL) and it handles both IPv4 and IPv6."),
                q("Em VPC, qual recurso é stateful?",
                  ...
                  "Subnet é um bloco de IPs e route table define destino — nenhum dos dois guarda estado de conexão.",
                  ...
                  "inbound AND outbound. A subnet is an IP block and a route table defines destinations — neither keeps connection state."),
                q("CIDR sobreposto entre VPCs causa:",
                  "Problema em peering, não é permitido.",
                  ["Backup automático entre as duas VPCs.",
                   "Aceleração do tráfego entre as VPCs.",
                   "Alta disponibilidade extra para a aplicação hospedada."],
                  "Pacotes não saberiam para qual VPC ir. Planeje CIDRs com IP plan global. "
                  "Sobreposição de faixa é um impedimento de roteamento, não um ganho de performance ou de redundância.",
                  statement_en="Overlapping CIDRs between VPCs cause:",
                  correct_en="A peering problem; it is not allowed.",
                  wrong_en=["Automatic backup between the two VPCs.",
                            "Acceleration of traffic between the VPCs.",
                            "Extra high availability for the hosted application."],
                  explanation_en="Packets would not know which VPC to go to. Plan CIDRs with a global IP plan. "
                  "Overlapping ranges are a routing blocker, not a performance or redundancy gain."),
            ],
        },
        # =====================================================================
...
"""
            ...
            "questions": [
                q("Security Group em AWS é:",
                  ...
                  ["Stateless, exige regra de retorno.",
                   "Aplicado ao host físico inteiro.",
                   "Restrito a IPv6."],
                  "Stateful = inbound libera o retorno do outbound automaticamente. NACL é o oposto. "
                  "O SG é avaliado por interface de rede (ENI), não por host, e cobre IPv4 e IPv6.",
                  statement_en="A Security Group in AWS is:",
                  correct_en="Stateful, automatic response.",
                  wrong_en=["Stateless, needs a return rule.",
                            "Applied to the whole physical host.",
                            "Restricted to IPv6."],
                  explanation_en="Stateful = inbound automatically allows the outbound return. NACL is the opposite. "
                  "The SG is evaluated per network interface (ENI), not per host, and covers both IPv4 and IPv6."),
                q("NACL em AWS é:",
                  "Stateless, precisa regras de saída e entrada.",
                  ["Restrito ao IPv4, sem cobrir IPv6.",
                   "Aplicado à instância, não à subnet.",
                   "Stateful, libera a resposta de volta automaticamente."],
                  "Esquecer regra outbound de portas efêmeras é fonte clássica de 'meu serviço não responde'. "
                  "A NACL age na borda da subnet e cobre IPv4 e IPv6; quem age na instância é o SG.",
                  statement_en="An NACL in AWS is:",
                  correct_en="Stateless, needs both outbound and inbound rules.",
                  wrong_en=["Restricted to IPv4, not covering IPv6.",
                            "Applied to the instance, not the subnet.",
                            "Stateful, it allows the return traffic automatically."],
                  explanation_en="Forgetting the outbound rule for ephemeral ports is the classic source of 'my service isn't responding'. "
                  "The NACL acts at the subnet boundary and covers IPv4 and IPv6; the SG is what acts at the instance."),
                q("Por padrão, SG inbound:",
                  "Bloqueia tudo.",
                  ["Libera tudo.",
                   "Libera SSH na porta 22.",
                   "Aceita as portas 80 e 443."],
                  "Default-deny, você precisa abrir explicitamente o que quer. "
                  "Nenhuma porta vem aberta de fábrica na entrada — o padrão permissivo fica só na saída.",
                  statement_en="By default, SG inbound:",
                  correct_en="Blocks everything.",
                  wrong_en=["Allows all.",
                            "Allows SSH on port 22.",
                            "Accepts ports 80 and 443."],
                  explanation_en="Default-deny, you need to explicitly open what you want. "
                  "No port comes open by default on inbound — the permissive default applies to outbound only."),
                q("Em SG, posso permitir tráfego vindo de outro SG?",
                  "Sim, referenciando o SG-id no source.",
                  ["Não, só por faixa de IP fixa.",
                   "Só via política de IAM, não pelo SG.",
                   "Só entre SGs da mesma subnet e mesma AZ."],
                  "Encadeamento por SG é a melhor prática para evitar IP hardcoded. "
                  "A referência funciona na VPC inteira (e em VPCs pareadas), sem depender de subnet, AZ ou IAM.",
                  statement_en="In an SG, can I allow traffic coming from another SG?",
                  correct_en="Yes, by referencing the SG-id as the source.",
                  wrong_en=["No, only by fixed IP range.",
                            "Only via IAM policy, not the SG.",
                            "Only between SGs in the same subnet and AZ."],
                  explanation_en="Chaining by SG is the best practice to avoid hard-coded IPs. "
                  "The reference works across the whole VPC (and peered VPCs), without depending on subnet, AZ or IAM."),
                q("NACL avalia regras:",
                  "Em ordem numérica até encontrar match.",
                  ["Considera a última regra cadastrada.",
                   "De forma aleatória entre as regras.",
                   "Pelo timestamp de criação de cada regra."],
                  "Por isso convenciona-se números (100, 200, 300...) e regra final 32766/deny all. "
                  "A ordem de cadastro e o horário de criação não influenciam: vale o número da regra.",
                  statement_en="An NACL evaluates rules:",
                  correct_en="In numeric order until it finds a match.",
                  wrong_en=["It considers the last rule registered.",
                            "Randomly among the existing rules.",
                            "By the creation timestamp of each rule."],
                  explanation_en="That's why numbering conventions (100, 200, 300...) plus a final deny-all rule at 32766 are used. "
                  "Registration order and creation time have no influence: the rule number decides."),
                q("Para HTTPS, qual porta liberar?",
                  ...
                  "80 TCP é o HTTP sem TLS (usado em redirecionamento) e 8443 TCP é porta alternativa comum em proxies.",
                  ...
                  "80 TCP is plain HTTP (used for redirects) and 8443 TCP is a common alternate proxy port."),
                q("SG aplica-se a:",
                  "Interfaces de rede (ENIs).",
                  ["Subnets inteiras.",
                   "Tabelas de rota da VPC.",
                   "Funções Lambda exclusivamente."],
                  "ENI pode ter até 5 SGs (limite ajustável). Lambda em VPC também usa SG da ENI. "
                  "Quem filtra no nível da subnet é a NACL; route table não filtra nada.",
                  statement_en="An SG applies to:",
                  correct_en="Network interfaces (ENIs).",
                  wrong_en=["Entire subnets.",
                            "The VPC's route tables.",
                            "Lambda functions exclusively."],
                  explanation_en="An ENI can have up to 5 SGs (adjustable limit). Lambda inside a VPC also uses the ENI's SG. "
                  "The NACL is what filters at the subnet level; a route table filters nothing."),
                q("Boas práticas com SG:",
                  "Granularidade alta, sem 0.0.0.0/0 desnecessário.",
                  ["Abrir 0.0.0.0/0 em qualquer porta.",
                   "Um único SG compartilhado pela infra inteira.",
                   "Manter as regras congeladas por meses após mudanças de escopo."],
                  "Granular = mais regras, mas auditável e principle of least access. "
                  "SG único vira permissão pelo mínimo denominador comum, e regra que não acompanha o "
                  "escopo acumula acesso que ninguém mais precisa.",
                  statement_en="Good practices with SGs:",
                  correct_en="High granularity, no unnecessary 0.0.0.0/0.",
                  wrong_en=["Open 0.0.0.0/0 on any port.",
                            "A single SG shared by the whole infrastructure.",
                            "Keep rules frozen for months after scope changes."],
                  explanation_en="Granular means more rules, but auditable and aligned with the principle of least access. "
                  "A single SG collapses into lowest-common-denominator permissions, and a rule that doesn't follow scope accumulates access nobody needs."),
                q("Permitir 0.0.0.0/0 em SSH é:",
                  "Risco crítico de força bruta.",
                  ["Boa prática em produção.",
                   "Bloqueia o serviço sshd na hora.",
                   "Requisito para o SSH funcionar."],
                  "Use bastion, VPN ou SSM Session Manager. SSH público em produção = manchete esperando acontecer. "
                  "Abrir a porta não impede o sshd de subir, e o protocolo funciona igual restrito a uma faixa.",
                  statement_en="Allowing 0.0.0.0/0 on SSH is:",
                  correct_en="A critical brute-force risk.",
                  wrong_en=["A good practice in production.",
                            "It blocks the sshd service at once.",
                            "A requirement for SSH to work."],
                  explanation_en="Use a bastion, VPN, or SSM Session Manager. Public SSH in production is a headline waiting to happen. "
                  "Opening the port doesn't stop sshd from starting, and the protocol works the same when restricted to a range."),
                q("Como auditar uso de SG?",
                  "VPC Flow Logs e CloudTrail.",
                  ["Consultar o histórico DNS.",
                   "Rodar o comando top no servidor.",
                   "Ler o log do sshd na instância."],
                  "Flow Logs mostra tráfego; CloudTrail mostra mudanças nas regras. "
                  "top mostra processo local, sshd cobre um serviço só e DNS não registra decisão de firewall.",
                  statement_en="How do you audit SG usage?",
                  correct_en="VPC Flow Logs and CloudTrail.",
                  wrong_en=["Check the DNS history.",
                            "Run top on the server.",
                            "Read sshd logs on the instance."],
                  explanation_en="Flow Logs show traffic; CloudTrail shows changes to the rules. "
                  "top shows local processes, sshd covers a single service, and DNS records no firewall decision."),
            ],
        },
        # =====================================================================
...
"""
            ...
            "questions": [
                q("Bucket público sem necessidade leva a:",
                  "Vazamento de dados.",
                  ["Custo menor.",
                   "Backup automático grátis.",
                   "Auto-scaling de armazenamento."],
                  "Maior parte dos vazamentos cloud nas últimas duas décadas começou em S3 público. "
                  "Deixar o bucket aberto não muda preço, não gera backup e não altera escalabilidade: "
                  "o S3 já escala e cobra igual sendo privado.",
                  ...
                  "Leaving the bucket open changes no pricing, creates no backup and alters no scaling: S3 already scales and bills the same when private."),
                q("Presigned URL serve para:",
                  "Conceder acesso temporário a um objeto sem expor credenciais.",
                  ["Dispensar a política de IAM do bucket.",
                   "Renomear arquivos em lote no bucket.",
                   "Acelerar o upload de arquivos grandes usando conexões paralelas."],
                  "TTL curto reduz risco. Limite método e tamanho para minimizar abuso. "
                  "A URL assinada carrega a permissão de quem a gerou — ela não substitui IAM, não renomeia "
                  "objeto e não muda a performance do upload (isso é multipart).",
                  statement_en="A presigned URL is used to:",
                  correct_en="Grant temporary access to an object without exposing credentials.",
                  wrong_en=["Remove the need for a bucket IAM policy.",
                            "Batch-rename files in the bucket.",
                            "Speed up large-file uploads using parallel connections."],
                  explanation_en="A short TTL reduces risk. Restrict method and size to minimize abuse. "
                  "The signed URL carries the permissions of whoever generated it — it doesn't replace IAM, rename objects, or change upload performance (that's multipart)."),
                q("Versionamento em S3 ajuda em:",
                  "Recuperação após exclusão acidental ou ransomware.",
                  ["Backup financeiro exigido por auditoria.",
                   "Aumento do número de IOPS do bucket.",
                   "Compressão automática de cada objeto armazenado no bucket."],
                  "Combine com MFA Delete e Object Lock para defesa em camadas. "
                  "Versionamento guarda revisões anteriores do objeto: não comprime, não muda desempenho "
                  "e não é um controle contábil.",
                  statement_en="Versioning in S3 helps with:",
                  correct_en="Recovery after accidental deletion or ransomware.",
                  wrong_en=["Financial backup required by an audit.",
                            "An increase in the bucket's IOPS.",
                            "Automatic compression of every object stored in the bucket."],
                  explanation_en="Combine it with MFA Delete and Object Lock for layered defense. "
                  "Versioning keeps previous revisions of the object: it doesn't compress, doesn't change performance, and isn't an accounting control."),
                q("SSE-KMS criptografa:",
                  "Objetos em repouso usando chaves do KMS.",
                  ["Só as tags associadas ao objeto.",
                   "Só o nome do arquivo, não o conteúdo.",
                   "O tráfego em trânsito entre cliente e servidor."],
                  "Trânsito é coberto por TLS (HTTPS). KMS adiciona granularidade, você pode "
                  "controlar quem usa cada chave. Nome e tags do objeto ficam em claro nos metadados.",
                  statement_en="SSE-KMS encrypts:",
                  correct_en="Objects at rest using KMS keys.",
                  wrong_en=["Only the tags attached to the object.",
                            "Only the file name, not the content.",
                            "Traffic in transit between client and server."],
                  explanation_en="Transit is covered by TLS (HTTPS). KMS adds granularity, letting you "
                  "control who uses each key. Object name and tags stay in the clear in the metadata."),
                q("'Block Public Access' na conta:",
                  "Garante que nenhum bucket fique exposto por engano.",
                  ["Aumenta o custo mensal do S3.",
                   "Apaga objetos já existentes nos buckets.",
                   "Bloqueia a criação de novas políticas de IAM na conta."],
                  "Quatro flags; ative as quatro a menos que tenha caso justificável de bucket público. "
                  "É um controle de exposição: não apaga objeto, não cobra nada e não interfere no IAM da conta.",
                  statement_en="'Block Public Access' at the account level:",
                  correct_en="Ensures no bucket ends up exposed by mistake.",
                  wrong_en=["Increases the monthly S3 cost.",
                            "Deletes objects that already exist in buckets.",
                            "Blocks the creation of new IAM policies in the account."],
                  explanation_en="Four flags; enable all four unless you have a justified case for a public bucket. "
                  "It's an exposure control: it deletes no object, costs nothing, and doesn't interfere with the account's IAM."),
                q("Lifecycle rule pode:",
                  "Mover objetos a Glacier após N dias.",
                  ["Habilitar HTTPS no bucket.",
                   "Renomear o bucket sem recriar.",
                   "Substituir a política de IAM aplicada ao bucket."],
                  "Reduz custo drasticamente para dados frios. Cuidado com custo de retrieval em Glacier. "
                  "Lifecycle age sobre o ciclo de vida do objeto (transição e expiração), não sobre nome "
                  "de bucket, protocolo ou permissão.",
                  statement_en="A lifecycle rule can:",
                  correct_en="Move objects to Glacier after N days.",
                  wrong_en=["Enable HTTPS on the bucket.",
                            "Rename the bucket without recreating it.",
                            "Replace the IAM policy applied to the bucket."],
                  explanation_en="It drastically cuts costs for cold data. Watch out for Glacier retrieval costs. "
                  "Lifecycle acts on the object's life cycle (transition and expiration), not on bucket name, protocol or permissions."),
                q("Para hospedar site estático em S3:",
                  "Habilite static website hosting + use CloudFront na frente.",
                  ["Use Route53 sozinho, sem outro serviço.",
                   "Use EC2 dedicada rodando nginx continuamente.",
                   "Use Lambda como servidor web, processando cada requisição HTTP."],
                  "CloudFront + Origin Access Identity permite manter o bucket privado. "
                  "Route53 só resolve nome; EC2 e Lambda funcionam, mas devolvem para você o servidor "
                  "que o S3 estático justamente elimina.",
                  statement_en="To host a static site on S3:",
                  correct_en="Enable static website hosting + put CloudFront in front.",
                  wrong_en=["Use Route53 by itself, with no other service.",
                            "Use a dedicated EC2 running nginx continuously.",
                            "Use Lambda as the web server, handling each HTTP request."],
                  explanation_en="CloudFront + Origin Access Identity lets you keep the bucket private. "
                  "Route53 only resolves names; EC2 and Lambda work, but hand you back the server that static S3 exists to remove."),
                q("Logs de acesso ao bucket vão para:",
                  "Outro bucket configurado como destino.",
                  ["O CloudWatch, sem bucket adicional.",
                   "O console da AWS, sem persistência.",
                   "Um índice interno do próprio bucket de origem."],
                  "Bucket de logs deve ser separado e com policy restritiva. Considere também CloudTrail data events. "
                  "O S3 server access logging entrega arquivos num bucket destino; não escreve em CloudWatch nem guarda índice interno.",
                  statement_en="Access logs for the bucket go to:",
                  correct_en="Another bucket configured as the destination.",
                  wrong_en=["CloudWatch, with no extra bucket.",
                            "The AWS console, with no persistence.",
                            "An internal index in the source bucket itself."],
                  explanation_en="The log bucket should be separate with a restrictive policy. Also consider CloudTrail data events. "
                  "S3 server access logging delivers files to a destination bucket; it writes neither to CloudWatch nor to an internal index."),
                q("Como evitar deleção acidental?",
                  "Object Lock + versionamento + MFA Delete.",
                  ["Backup local fora da AWS.",
                   "Política de IAM restritiva na conta.",
                   "Replicação cross-region do bucket inteiro."],
                  "Object Lock em modo Compliance impede até root de apagar antes do TTL. "
                  "IAM ajuda, mas quem tem permissão ainda apaga; e replicação copia a deleção "
                  "para o destino se o versionamento não estiver no caminho.",
                  statement_en="How do you prevent accidental deletion?",
                  correct_en="Object Lock + versioning + MFA Delete.",
                  wrong_en=["A local backup outside AWS.",
                            "A restrictive IAM policy on the account.",
                            "Cross-region replication of the whole bucket."],
                  explanation_en="Object Lock in Compliance mode prevents even root from deleting before the retention period. "
                  "IAM helps, but whoever holds the permission still deletes; and replication carries the deletion to the destination when versioning isn't in the way."),
                q("S3 tem garantia de durabilidade nominal de:",
                  "11 noves (99.999999999%).",
                  ["3 noves (99.9%).",
                   "5 noves (99.999%).",
                   "Sem garantia formal documentada."],
                  "Calculado replicando objetos cross-AZ. Disponibilidade é menor (4 noves no Standard). "
                  "3 e 5 noves são números de disponibilidade típicos de outros serviços, não de durabilidade do S3.",
                  statement_en="S3's nominal durability guarantee is:",
                  correct_en="11 nines (99.999999999%).",
                  wrong_en=["3 nines (99.9%).",
                            "5 nines (99.999%).",
                            "No formal documented guarantee."],
                  explanation_en="Calculated by replicating objects cross-AZ. Availability is lower (4 nines on Standard). "
                  "3 and 5 nines are typical availability figures for other services, not S3 durability."),
            ],
        },
        # =====================================================================
            ...
            "questions": [
                ...
                q("TLS depende de:",
                  ...
                  ["O endereço IP do servidor.",
                   "Regra de firewall na porta 443.",
                   "A resolução de nome feita pelo DNS."],
                  "Cliente verifica o certificado contra CAs confiáveis; servidor prova posse da chave privada. "
                  "DNS e firewall são pré-requisitos de conectividade, não de identidade criptográfica.",
                  statement_en="TLS depends on:",
                  correct_en="An X.509 certificate and a private key.",
                  wrong_en=["The server's IP address.",
                            "A firewall rule on port 443.",
                            "The name resolution done by DNS."],
                  explanation_en="The client checks the certificate against trusted CAs; the server proves it holds the private key. "
                  "DNS and firewall are connectivity prerequisites, not cryptographic identity."),
                q("Por que rotacionar chaves KMS?",
                  "Reduz impacto de eventual comprometimento.",
                  ["Requisito do protocolo HTTP.",
                   "Reduz o custo mensal do KMS.",
                   "Aumenta a velocidade da chamada de API cifrada."],
                  "KMS rotaciona automaticamente em CMKs gerenciadas (anual). Re-criptografa lazy ao acessar. "
                  "Rotação é controle de risco: não muda preço, protocolo nem latência da operação.",
                  statement_en="Why rotate KMS keys?",
                  correct_en="It reduces the impact of a potential compromise.",
                  wrong_en=["A requirement of the HTTP protocol.",
                            "It reduces the monthly KMS cost.",
                            "It increases the speed of the encrypted API call."],
                  explanation_en="KMS rotates automatically on managed CMKs (yearly). It re-encrypts lazily on access. "
                  "Rotation is risk control: it changes neither price, protocol, nor operation latency."),
                q("HSM serve para:",
                  "Armazenar chaves criptográficas em hardware dedicado.",
                  ["Criar políticas de acesso no IAM da conta.",
                   "Dispensar a configuração de IAM na conta.",
                   "Comprimir o dado antes de gravá-lo fisicamente no disco."],
                  "HSMs têm certificações (FIPS 140-3 Level 3) e impedem extração da chave. "
                  "Autorização continua sendo trabalho do IAM, e compressão é problema de storage, não de HSM.",
                  statement_en="An HSM is used for:",
                  correct_en="Storing cryptographic keys in dedicated hardware.",
                  wrong_en=["Creating access policies in the account's IAM.",
                            "Removing the need to configure IAM on the account.",
                            "Compressing data before physically writing it to disk."],
                  explanation_en="HSMs carry certifications (FIPS 140-3 Level 3) and prevent key extraction. "
                  "Authorization is still IAM's job, and compression is a storage concern, not an HSM one."),
                q("Algoritmo recomendado para hash de senha:",
                  ...
                  "MD5 e SHA-256 são rápidos demais (bilhões de tentativas por segundo em GPU) mesmo com salt, "
                  "e Base64 é codificação reversível, não hash.",
                  statement_en="Recommended algorithm for password hashing:",
                  correct_en="Argon2 or bcrypt with a high cost factor.",
                  wrong_en=["MD5 with salt.",
                            "Base64 with salt.",
                            "SHA-256 with a random per-user salt."],
                  explanation_en="Argon2 won the Password Hashing Competition. bcrypt remains acceptable. "
                  "MD5 and SHA-256 are far too fast (billions of guesses per second on a GPU) even with salt, and Base64 is reversible encoding, not a hash."),
                ...
                q("HSTS é mecanismo de:",
                  ...
                  ["Roteia pacotes IP.",
                   "Faz backup do certificado.",
                   "Cifra dado parado em disco."],
                  "Header HTTP que diz: 'sempre acesse este host por HTTPS pelos próximos N segundos'. "
                  "É instrução para o navegador, não roteamento, não backup e não criptografia em repouso.",
                  statement_en="HSTS is a mechanism for:",
                  correct_en="Forcing HTTPS in browsers.",
                  wrong_en=["Routing IP packets.",
                  ...
                  "It's an instruction to the browser, not routing, not backup and not encryption at rest."),
                q("Certificate Authority (CA) confiável é necessária para:",
                  "Que clientes confiem no certificado sem warning.",
                  ["Melhorar a performance do servidor web.",
                   "Servir conteúdo em HTTP puro.",
                   "Comprimir os dados antes de enviá-los ao cliente pela rede."],
                  "Em produção pública use Let's Encrypt/ACM/etc. Em interno, considere CA privada (Vault, AWS PCA). "
                  "A CA resolve confiança na identidade: não acelera o servidor nem comprime resposta, e HTTP puro dispensa certificado.",
                  ...
                  wrong_en=["Improving web server performance.",
                            "Serving content over plain HTTP.",
                            "Compressing data before sending it to the client over the network."],
                  explanation_en="In public production use Let's Encrypt/ACM/etc. Internally, consider a private CA (Vault, AWS PCA). "
                  "A CA solves identity trust: it neither speeds up the server nor compresses responses, and plain HTTP needs no certificate."),
                q("Vazou a chave privada do TLS, deve-se:",
                  "Revogar e rotacionar imediatamente.",
                  ["Abrir chamado de SLA com a CA.",
                   "Manter em uso por compatibilidade.",
                   "Apagar os logs antigos de uso da chave."],
                  "Tráfego antigo pode ser descriptografado se não houver forward secrecy. Revogue via CRL/OCSP. "
                  "Apagar log destrói a evidência do incidente, e manter a chave em uso mantém o atacante dentro.",
                  statement_en="If the TLS private key leaks, you should:",
                  correct_en="Revoke and rotate it immediately.",
                  wrong_en=["Open an SLA ticket with the CA.",
                            "Keep it in use for compatibility.",
                            "Delete old logs of the key's use."],
                  explanation_en="Old traffic can be decrypted if there's no forward secrecy. Revoke it via CRL/OCSP. "
                  "Deleting logs destroys incident evidence, and keeping the key in use keeps the attacker inside."),
                q("'Encryption at rest' protege:",
                  "Dados armazenados em disco.",
                  ["Dados na memória RAM.",
                   "A resolução de nome via DNS.",
                   "Tráfego entre dois servidores."],
                  "Mitiga roubo de disco/snapshot. Não protege se atacante já tem acesso lógico ao recurso. "
                  "Tráfego entre servidores é coberto por TLS (em trânsito), e dado em RAM está em claro por definição.",
                  statement_en="'Encryption at rest' protects:",
                  correct_en="Data stored on disk.",
                  wrong_en=["Data in RAM memory.",
                  ...
                  "Traffic between servers is covered by TLS (in transit), and data in RAM is in the clear by definition."),
            ],
        },
        # =====================================================================
            ...
            "questions": [
                q("Os '4 golden signals' são:",
                  ...
                  ["CPU, RAM, disco e rede.",
                   "Uptime, custo, tickets e deploys.",
                   "Requisições, quadros, bytes e milissegundos."],
                  "Definidos pelo Google SRE Book. Cobrem as dimensões essenciais de qualquer serviço. "
                  "CPU e RAM são métricas de recurso (entram como saturação), e as demais listas misturam "
                  "unidades com sinais, sem cobrir erro nem latência.",
                  statement_en="The '4 golden signals' are:",
                  correct_en="Latency, traffic, errors, and saturation.",
                  wrong_en=["CPU, RAM, disk, and network.",
                            "Uptime, cost, tickets, and deploys.",
                            "Requests, frames, bytes, and milliseconds."],
                  explanation_en="Defined by the Google SRE Book. They cover the essential dimensions of any service. "
                  "CPU and RAM are resource metrics (they show up as saturation), and the other lists mix units with signals, covering neither errors nor latency."),
                q("SLO mede:",
                  "Objetivo de qualidade de serviço (ex.: 99,9% de disponibilidade).",
                  ["Custo mensal da conta de cloud.",
                   "Contagem de bugs no issue tracker.",
                   "Latência de uma requisição isolada, medida num único instante do dia."],
                  "É o objetivo interno; SLA é o contrato com cliente (geralmente mais conservador). "
                  "O SLO é um alvo agregado numa janela de tempo, não a medição pontual de uma requisição.",
                  statement_en="An SLO measures:",
                  correct_en="A service quality objective (e.g., 99.9% availability).",
                  wrong_en=["The cloud account's monthly cost.",
                            "The count of bugs in the issue tracker.",
                            "The latency of one isolated request, measured at a single point in the day."],
                  explanation_en="It's the internal target; the SLA is the contract with the customer (usually more conservative). "
                  "An SLO is an aggregate target over a time window, not a point measurement of one request."),
                q("Diferença SLI vs SLO:",
                  "SLI é a medida (indicador), SLO é o objetivo.",
                  ["São sinônimos em ferramentas diferentes.",
                   "O SLO é a métrica bruta do sistema.",
                   "O SLI é o contrato formal assinado com o cliente."],
                  "Você mede SLI; SLO é o limite que diz se está bom ou ruim. "
                  "Contrato com cliente é o SLA — um terceiro termo, com consequência comercial.",
                  statement_en="Difference between SLI and SLO:",
                  correct_en="SLI is the measurement (indicator), SLO is the target.",
                  wrong_en=["They're synonyms across different tools.",
                            "The SLO is the raw metric from the system.",
                            "The SLI is the formal contract signed with the customer."],
                  explanation_en="You measure the SLI; the SLO is the threshold that says whether it's good or bad. "
                  "The customer contract is the SLA — a third term, with commercial consequences."),
                ...
                q("PromQL é linguagem de:",
                  ...
                  ["Formato de log.",
                   "Script Python de alarme.",
                   "Um dialeto de SQL para séries temporais."],
                  "Permite agregações, taxas (`rate(...)[5m]`), percentis (`histogram_quantile`). "
                  "Não é SQL nem derivada dele, não é linguagem de script e não descreve formato de log.",
                  statement_en="PromQL is a language for:",
                  correct_en="Querying Prometheus.",
                  wrong_en=["A log format.",
                            "A Python alarm script.",
                            "A SQL dialect for time series."],
                  explanation_en="Allows aggregations, rates (`rate(...)[5m]`), and percentiles (`histogram_quantile`). "
                  "It is not SQL nor derived from it, is not a scripting language, and describes no log format."),
                q("Histograma em métricas serve para:",
                  "Distribuir valores em buckets e calcular percentis (p99 etc.).",
                  ["Substituir o trace distribuído da aplicação.",
                   "Contar requisições recebidas, sem estatística.",
                   "Listar cada erro individualmente, sem qualquer agregação do resultado."],
                  "Histograma é local (cliente). Para somar entre instâncias, use buckets compatíveis. "
                  "Contador simples não dá distribuição, listar evento cru é papel de log, e trace resolve outro problema (causalidade).",
                  ...
                  wrong_en=["Replace the application's distributed tracing.",
                            "Count received requests, with no statistics.",
                            "List each error individually, with no aggregation of the result."],
                  explanation_en="A histogram is local (client-side). To sum across instances, use compatible buckets. "
                  "A plain counter gives no distribution, listing raw events is the log's job, and tracing solves a different problem (causality)."),
                q("Alert fadiga ocorre quando:",
                  "Há tantos alertas que ninguém presta atenção.",
                  ["O SLA contratado aumenta.",
                   "O Grafana atualiza o dashboard sozinho.",
                   "O Prometheus cai e para de coletar métricas novas."],
                  "Cada alerta sem ação útil corrói a confiança no sistema. Limpe agressivamente. "
                  "Coletor fora do ar é ausência de sinal, o problema oposto: fadiga é excesso de sinal sem ação.",
                  statement_en="Alert fatigue happens when:",
                  correct_en="So many alerts fire that nobody pays attention.",
                  wrong_en=["The contracted SLA increases.",
                            "Grafana updates the dashboard on its own.",
                            "Prometheus goes down and stops collecting new metrics."],
                  explanation_en="Every alert with no useful action erodes trust in the system. Prune aggressively. "
                  "A collector going down is absence of signal, the opposite problem: fatigue is excess signal with no action."),
                q("Métrica vs log vs trace:",
                  ...
                  ["São a mesma coisa com nomes diferentes.",
                   "Métrica e trace são o mesmo conceito.",
                   "Só o trace importa; métrica e log são dispensáveis quando há tracing."],
                  "Os três são pilares da observabilidade, complementares, não substitutos. "
                  "Trace mostra o caminho de uma requisição; métrica mostra tendência agregada e log guarda o detalhe do evento.",
                  ...
                  wrong_en=["They're the same thing with different names.",
                            "Metric and trace are the same concept.",
                            "Only the trace matters; metric and log are dispensable when tracing exists."],
                  explanation_en="The three are pillars of observability, complementary, not substitutes. "
                  "A trace shows one request's path; a metric shows aggregate trend and a log keeps the event detail."),
                q("SLO de 99,99% permite quantos minutos de downtime/mês?",
                  "Cerca de 4,3 minutos.",
                  ["Cerca de 4,3 horas.",
                   "Cerca de 43 minutos.",
                   "Cerca de 43 segundos."],
                  "30d x 24h x 60min x 0,01% ≈ 4,3 min. Quatro noves é caro. "
                  "43 minutos corresponde a 99,9% (três noves) e 4,3 horas, a 99% — cada nove a mais divide a janela por dez.",
                  statement_en="How many minutes of downtime/month does a 99.99% SLO allow?",
                  correct_en="About 4.3 minutes.",
                  wrong_en=["About 4.3 hours.",
                            "About 43 minutes.",
                            "About 43 seconds."],
                  explanation_en="30d x 24h x 60min x 0.01% ≈ 4.3 min. Four nines is expensive. "
                  "43 minutes corresponds to 99.9% (three nines) and 4.3 hours to 99% — each extra nine divides the window by ten."),
                q("Cardinalidade alta em métricas causa:",
                  "Custo crescente e degradação do backend.",
                  ["Backup automático das métricas.",
                   "Resolução automática do problema.",
                   "Consultas mais rápidas no banco de métricas."],
                  "Cada combinação única de labels é uma série. Evite user_id/request_id em métricas. "
                  "Mais séries significa mais índice para varrer: a consulta fica mais lenta, não mais rápida.",
                  statement_en="High cardinality in metrics causes:",
                  correct_en="Growing cost and backend degradation.",
                  wrong_en=["Automatic backup of the metrics.",
                            "Automatic resolution of the problem.",
                            "Faster queries against the metrics database."],
                  explanation_en="Every unique combination of labels is a series. Avoid user_id/request_id in metrics. "
                  "More series means more index to scan: queries get slower, not faster."),
            ],
        },
        # =====================================================================
        # 2.9 Backup & DR
...
"""
            ...
            "questions": [
                q("RPO mede:",
                  ...
                  ["O tempo até o serviço voltar.",
                   "O custo mensal da conta de cloud.",
                   "A velocidade de leitura e escrita do disco do servidor."],
                  "RPO baixo = backups frequentes. RPO=0 só com replicação síncrona, e cara. "
                  "Tempo até o serviço voltar é o RTO — o par que sempre aparece junto do RPO.",
                  statement_en="RPO measures:",
                  correct_en="How much data is acceptable to lose in an incident.",
                  wrong_en=["The time until the service is back.",
                            "The cloud account's monthly cost.",
                            "The read and write speed of the server's disk."],
                  explanation_en="Low RPO = frequent backups. RPO=0 only with synchronous replication, and it's expensive. "
                  "Time until the service is back is the RTO — the metric that always shows up next to RPO."),
                q("RTO mede:",
                  "Tempo aceitável para retomar a operação.",
                  ["A latência entre cliente e servidor.",
                   "O throughput máximo da aplicação.",
                   "A quantidade de dado perdido numa falha grave."],
                  "RTO baixo = automação. Atender RTO de minutos manualmente é impossível. "
                  "Quantidade de dado perdido é o RPO; latência e throughput são métricas de desempenho, não de recuperação.",
                  statement_en="RTO measures:",
                  correct_en="The acceptable time to resume operation.",
                  wrong_en=["The latency between client and server.",
                            "The application's maximum throughput.",
                  ...
                  "The amount of data lost is the RPO; latency and throughput are performance metrics, not recovery ones."),
                q("Regra 3-2-1 sugere:",
                  "3 cópias, em 2 mídias, com 1 offsite.",
                  ["3 réplicas no mesmo disco físico.",
                   "1 backup semanal, sem cópia extra.",
                   "2 cópias no mesmo datacenter, mais um índice."],
                  "Sobrevive a incêndio do datacenter, ransomware na conta principal e mídia falha. "
                  "Cópias que dividem disco ou datacenter caem juntas: o valor da regra está justamente na separação física.",
                  statement_en="The 3-2-1 rule suggests:",
                  correct_en="3 copies, on 2 media, with 1 offsite.",
                  wrong_en=["3 replicas on the same physical disk.",
                            "1 weekly backup, with no extra copy.",
                            "2 copies in the same datacenter, plus an index."],
                  explanation_en="Survives a datacenter fire, ransomware on the main account, and media failure. "
                  "Copies sharing a disk or a datacenter fall together: the rule's value is precisely the physical separation."),
                q("Snapshot incremental:",
                  "Salva só as mudanças desde o último snapshot.",
                  ["Substitui o backup full mais recente.",
                   "Salva o conteúdo inteiro a cada execução.",
                   "Apaga o snapshot mais antigo a cada execução do processo."],
                  "Eficiente em espaço, mas restore depende da cadeia de incrementais. "
                  "Copiar tudo de novo é o backup full, e expiração de snapshot antigo é regra de retenção separada.",
                  statement_en="Incremental snapshot:",
                  correct_en="Saves only the changes since the last snapshot.",
                  wrong_en=["Replaces the most recent full backup.",
                            "Saves the whole content on every run.",
                            "Deletes the oldest snapshot on every run of the process."],
                  explanation_en="Space-efficient, but restore depends on the chain of incrementals. "
                  "Copying everything again is the full backup, and expiring the oldest snapshot is a separate retention rule."),
                q("Backup sem teste é:",
                  "Inútil, só descobre o problema na hora.",
                  ["Rápido de restaurar em qualquer caso.",
                   "Garantia legal perante auditoria.",
                   "Suficiente para cumprir o compliance da empresa."],
                  "Game day mensal/trimestral revela schemas que mudaram, credenciais que expiraram, etc. "
                  "Auditoria e compliance costumam exigir evidência de restauração testada, não só a existência do arquivo.",
                  statement_en="A backup without testing is:",
                  correct_en="Useless, you only find the problem when it's too late.",
                  wrong_en=["Fast to restore in any case.",
                            "A legal guarantee before an audit.",
                            "Enough to satisfy the company's compliance."],
                  explanation_en="Monthly/quarterly game days reveal changed schemas, expired credentials, etc. "
                  "Audit and compliance usually demand evidence of a tested restore, not just the existence of the file."),
                q("DR em região alternativa serve para:",
                  "Mitigar falha de uma região inteira.",
                  ["Aumentar o custo do backup.",
                   "Trocar de provedor de cloud.",
                   "Reduzir a latência para o usuário final."],
                  "Region failures são raros mas existem (us-east-1 outages). "
                  "Reduzir latência é objetivo de CDN ou de deploy multi-região ativo, e mudar de provedor é outra discussão (portabilidade).",
                  statement_en="DR in an alternate region serves to:",
                  correct_en="Mitigate the failure of an entire region.",
                  wrong_en=["Increase backup cost.",
                  ...
                  "Reducing latency is the goal of a CDN or an active multi-region deploy, and switching providers is a different discussion (portability)."),
                q("Encriptação de backup é:",
                  "Obrigatória, backup vaza, dado vaza.",
                  ["Necessária só para dado de banco.",
                   "Opcional, conforme política interna.",
                   "Inútil, o isolamento de rede já protege o backup."],
                  "Backup é onde quase todo dado está, junto. KMS + bucket policy restritiva. "
                  "Isolamento de rede não acompanha a mídia: fita, snapshot exportado e cópia offsite saem do perímetro.",
                  statement_en="Encrypting a backup is:",
                  correct_en="Mandatory, if the backup leaks, the data leaks.",
                  wrong_en=["Needed only for database data.",
                            "Optional, per internal policy.",
                            "Useless, network isolation already protects the backup."],
                  explanation_en="Backup is where almost all the data sits, together. KMS plus a restrictive bucket policy. "
                  "Network isolation doesn't travel with the media: tape, exported snapshots and offsite copies leave the perimeter."),
                q("Game day em DR é:",
                  "Simulação real para validar runbooks.",
                  ["Confraternização do time de infra.",
                   "Hackathon interno de produto.",
                   "Auditoria fiscal conduzida por contador externo."],
                  "Documente lições aprendidas e ajuste runbook após cada game day. "
                  "O nome sugere descontração, mas o exercício é operacional: falhar em ambiente controlado antes de falhar em produção.",
                  statement_en="A game day in DR is:",
                  correct_en="A real simulation to validate runbooks.",
                  wrong_en=["A get-together for the infra team.",
                            "An internal product hackathon.",
                            "A tax audit conducted by an external accountant."],
                  explanation_en="Document lessons learned and adjust the runbook after each game day. "
                  "The name sounds playful, but the exercise is operational: fail in a controlled environment before failing in production."),
                q("Pilot light é:",
...
"""
            ...
            "questions": [
                q("Tag obrigatória recomendada:",
                  ...
                  ["Name, Region, Type.",
                   "Nome do recurso e região.",
                   "Data de criação e ID interno da conta."],
                  "Sem tags consistentes, atribuição de custo vira política em vez de matemática. "
                  "Nome, região e data já vêm do próprio recurso; o que falta é quem paga e para qual ambiente.",
                  statement_en="Recommended mandatory tag:",
                  correct_en="Owner, Environment, CostCenter.",
                  wrong_en=["Name, Region, Type.",
                            "Resource name and region.",
                            "Creation date and internal account ID."],
                  explanation_en="Without consistent tags, cost attribution becomes politics instead of math. "
                  "Name, region and date already come from the resource itself; what's missing is who pays and for which environment."),
                q("Spot Instance é:",
                  "VM barata que pode ser interrompida.",
                  ["Disco SSD de alta performance.",
                   "Um tipo de role no IAM da conta.",
                   "VM premium com disponibilidade garantida."],
                  "Aviso de 2 minutos antes da interrupção. Ideal para batch, CI, treino de ML. "
                  "O desconto existe justamente porque a capacidade é cedida de volta quando o provedor precisa dela.",
                  statement_en="A Spot Instance is:",
                  correct_en="A cheap VM that can be interrupted.",
                  wrong_en=["A high-performance SSD disk.",
                            "A type of role in the account's IAM.",
                            "A premium VM with guaranteed availability."],
                  explanation_en="2-minute warning before interruption. Ideal for batch, CI, ML training. "
                  "The discount exists precisely because the capacity is taken back when the provider needs it."),
                q("Reserved Instance dá desconto se:",
                  "Você se compromete por 1 ou 3 anos.",
                  ["Você usa IPv6 na instância.",
                   "Você paga à vista, sem compromisso.",
                   "Você deixa a instância parada boa parte do período."],
                  "Savings Plans são mais flexíveis (qualquer family/region) com desconto similar. "
                  "Pagar adiantado aumenta o desconto, mas o gatilho é o compromisso de prazo — e instância parada não gera economia de RI.",
                  statement_en="A Reserved Instance gives a discount if:",
                  correct_en="You commit for 1 or 3 years.",
                  wrong_en=["You use IPv6 on the instance.",
                            "You pay upfront, with no commitment.",
                            "You leave the instance stopped for most of the period."],
                  explanation_en="Savings Plans are more flexible (any family/region) with a similar discount. "
                  "Paying upfront raises the discount, but the trigger is the term commitment — and a stopped instance saves nothing on an RI."),
                q("Recurso órfão é:",
                ...
                q("Right-sizing é:",
                  ...
                  ["Escolher o menor tamanho disponível.",
                   "Escolher o maior tamanho por segurança.",
                   "Ajustar a largura do disco da instância provisionada."],
                  "Ferramentas mostram CPU/memória médio e sugerem família/tamanho menor. "
                  "Right-sizing olha o consumo medido: encolher às cegas degrada o serviço tanto quanto superdimensionar desperdiça.",
                  statement_en="Right-sizing is:",
                  correct_en="Adjusting instance size to match real usage.",
                  wrong_en=["Choosing the smallest size available.",
                            "Choosing the largest size for safety.",
                            "Adjusting the disk width of the provisioned instance."],
                  explanation_en="Tools show average CPU/memory and suggest a smaller family/size. "
                  "Right-sizing looks at measured consumption: shrinking blindly degrades the service as much as oversizing wastes money."),
                q("Budget alerts servem para:",
                  "Avisar antes do orçamento estourar.",
                  ["Aumentar a cota automaticamente.",
                   "Reduzir a latência do usuário final.",
                   "Acelerar o deploy de uma aplicação nova."],
                  "Configure alertas em 50%/80%/100% para evitar surpresa no fim do mês. "
                  "O alerta é informativo: ele não corta gasto nem ajusta cota sozinho — a ação continua sendo do time.",
                  statement_en="Budget alerts serve to:",
                  correct_en="Warn before the budget is blown.",
                  wrong_en=["Raise the quota automatically.",
                            "Reduce end-user latency.",
                            "Speed up the deploy of a new application."],
                  explanation_en="Configure alerts at 50%/80%/100% to avoid an end-of-month surprise. "
                  "The alert is informational: it neither cuts spend nor adjusts quota by itself — acting is still the team's job."),
                q("FinOps maturity vai do crawl ao:",
                  ...
                  "Walk é o estágio do meio, não o final; jog e sprint não fazem parte do vocabulário do FinOps Framework.",
                  ...
                  "Walk is the middle stage, not the final one; jog and sprint aren't part of the FinOps Framework vocabulary."),
                q("Para batches diários:",
                  "Considere Spot/Preemptible.",
                  ["Use on-demand contínuo.",
                   "Rode em hardware próprio.",
                   "Contrate Reserved Instance de 3 anos."],
                  "Batch tolera interrupção; spot economiza até 90% comparado a on-demand. "
                  "Reserva de 3 anos cobra a capacidade o tempo todo para uma carga que roda minutos por dia.",
                  statement_en="For daily batches:",
                  correct_en="Consider Spot/Preemptible.",
                  wrong_en=["Use continuous on-demand.",
                            "Run on your own hardware.",
                            "Buy a 3-year Reserved Instance."],
                  explanation_en="Batch tolerates interruption; spot saves up to 90% compared to on-demand. "
                  "A 3-year reservation pays for capacity around the clock for a job that runs minutes a day."),
                q("Auto Scaling reduz custo porque:",
                  "Provisiona apenas quando há demanda.",
                  ["Aumenta a cota da conta.",
                   "Substitui a política de IAM da instância.",
                   "Libera uma configuração extra de segurança."],
                  "Combine com warm pool para reduzir cold start e ainda economizar. "
                  "Auto Scaling mexe na quantidade de instâncias: não altera cota, permissão nem postura de segurança.",
                  statement_en="Auto Scaling reduces cost because:",
                  correct_en="It provisions only when there's demand.",
                  wrong_en=["It increases the account quota.",
                            "It replaces the instance's IAM policy.",
                            "It unlocks an extra security configuration."],
                  explanation_en="Combine with a warm pool to reduce cold start and still save money. "
                  "Auto Scaling changes the number of instances: it alters neither quota, permissions, nor security posture."),
                q("Showback vs chargeback:",
                  "Showback mostra; chargeback cobra internamente.",
                  ["Ambos cobram o cliente externo.",
                   "São a mesma prática com nomes distintos.",
                   "Showback cobra internamente e chargeback só relata o gasto."],
                  "Showback gera consciência. Chargeback cria accountability, recomendado em "
                  "estágios mais maduros de FinOps. Os dois olham para dentro da empresa: nenhum deles "
                  "chega à fatura do cliente final.",
                  statement_en="Showback vs chargeback:",
                  correct_en="Showback shows the cost; chargeback bills it internally.",
                  wrong_en=["Both bill the external customer.",
                            "The same practice with different names.",
                            "Showback bills internally and chargeback only reports the spend."],
                  explanation_en="Showback builds awareness. Chargeback creates accountability, recommended in more mature FinOps stages. "
                  "Both look inward: neither reaches the end customer's invoice."),
            ],
        },
    ],
}
