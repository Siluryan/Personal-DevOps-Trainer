...
admins.</p>""",
            ...
            "questions": [
                q("`git rebase` faz:",
                  ...
                  ["Mescla os branches criando um commit de merge extra.",
                   "Apaga o branch e o histórico associado a ele.",
                   "Sincroniza o repositório local com o remoto sem criar commit."],
                  "Rebase gera novos hashes; não use em história já compartilhada. Merge, ao contrário, "
                  "cria um commit extra e preserva a história original; quem sincroniza com o "
                  "remoto é `git fetch`/`git pull`, que não reescreve nada.",
                  statement_en="`git rebase` does:",
                  correct_en="Reapplies commits onto another base, rewriting history.",
                  wrong_en=["Merges the branches, creating an extra merge commit.",
                            "Deletes the branch and its associated history.",
                ...
                q("Por que evitar force push em main?",
                  ...
                  ["É mais lento que um push comum em repos grandes.",
                   "Exige um token de administrador da organização.",
                   "Falha silenciosamente em repositórios hospedados no GitHub."],
                  "Qualquer dev com a história anterior fica desincronizado e pode perder commits. "
                  "O custo não é técnico: force push não é mais lento, não pede token especial e "
                  "funciona em qualquer host Git — o estrago é na história compartilhada.",
                  statement_en="Why avoid force-pushing to main?",
                  correct_en="It can rewrite shared history and break the team's work.",
                  wrong_en=["It's slower than a regular push on big repos.",
                            "It requires an organization administrator token.",
                            "It fails silently on repositories hosted on GitHub."],
                  explanation_en="Any dev with the previous history becomes out of sync and can lose commits. "
                  "The cost isn't technical: force push isn't slower, needs no special token and "
                  "works on any Git host — the damage is to the shared history."),
                q("Signed commits servem para:",
                  "Provar autoria via GPG/SSH.",
                  ["Acelerar o push ao remoto.",
                   "Dispensar o branch de trabalho.",
                   "Comprimir o commit antes do envio."],
                  "Evita spoofing, atacante consegue dizer 'commit do CTO' sem assinatura. Com GPG, "
                  "GitHub mostra 'Verified'. A assinatura é metadado do commit: não muda a "
                  "velocidade do push, o tamanho do objeto nem o fluxo de branches.",
                  statement_en="Signed commits are used to:",
                  correct_en="Prove authorship via GPG/SSH.",
                  wrong_en=["Speed up the push to the remote.",
                            "Skip the working branch.",
                            "Compress the commit before sending it."],
                  explanation_en="Prevents spoofing — without a signature, an attacker can claim a commit is "
                  "'from the CTO'. With GPG, GitHub shows 'Verified'. The signature is commit "
                  "metadata: it changes neither push speed, object size, nor the branching flow."),
                q("Em PR, o que é review obrigatório?",
                  "Regra que exige aprovação humana antes do merge.",
                  ["Bloqueia o repositório para novas alterações.",
                   "Conta como um deploy feito em produção.",
                   "Pula a execução do CI configurado no repositório."],
                  "Combinada com CODEOWNERS, garante que pessoas certas sejam ouvidas. A regra age no "
                  "merge, não no repositório: o push para branches segue liberado, o CI continua "
                  "rodando e aprovar um PR não publica nada em produção.",
                  statement_en="In a PR, what does a required review mean?",
                  correct_en="A rule requiring human approval before the merge.",
                  wrong_en=["It locks the repository against new changes.",
                            "It counts as a deploy made to production.",
                            "It skips running the CI configured for that repository."],
                  explanation_en="Combined with CODEOWNERS, it guarantees the right people get heard. The rule "
                  "acts on the merge, not the repository: pushing to branches stays open, CI keeps "
                  "running, and approving a PR ships nothing to production."),
                q("Diferença entre merge e rebase:",
                  "Merge preserva história; rebase a reescreve linearmente.",
                  ["Os dois comandos são idênticos na prática.",
                   "O rebase é mais lento de executar que o merge.",
                   "O merge apaga os commits mais antigos do branch de origem."],
                  "Escolha conforme política do time. Misturar pode confundir o histórico. Nenhum dos "
                  "dois apaga commits do branch de origem, e a diferença de desempenho entre eles "
                  "é irrelevante — o que muda é a forma do grafo resultante.",
                  statement_en="Difference between merge and rebase:",
                  correct_en="Merge preserves history; rebase rewrites it linearly.",
                  wrong_en=["The two commands are identical in practice.",
                            "Rebase is slower to run than a merge.",
                            "Merge deletes the oldest commits on the source branch."],
                  explanation_en="Pick one based on team policy. Mixing them can confuse the history. Neither "
                  "deletes commits from the source branch, and the performance difference between "
                  "them is irrelevant — what changes is the shape of the resulting graph."),
                q("`.gitignore` serve para:",
                  "Listar arquivos a não rastrear (ex.: .env).",
                  ["Apagar do histórico um arquivo já commitado.",
                   "Bloquear um push para o branch principal.",
                   "Substituir o arquivo LICENSE do projeto."],
                  "Para arquivos já rastreados, é preciso `git rm --cached` antes. O arquivo age no "
                  "índice, na hora de rastrear: ele não reescreve histórico, não bloqueia push "
                  "e não tem relação com LICENSE ou outros metadados do projeto.",
                  statement_en="`.gitignore` is used to:",
                  correct_en="List files that should not be tracked (e.g. .env).",
                  wrong_en=["Erase a file that was already committed from history.",
                            "Block a push to the main branch.",
                            "Replace the project's LICENSE file with another."],
                  explanation_en="For files already tracked, you need `git rm --cached` first. The file acts on "
                  "the index, at tracking time: it rewrites no history, blocks no push, and has "
                  "nothing to do with LICENSE or other project metadata."),
                q("Trunk-based development prefere:",
                  "Branches curtas e merge frequente em main.",
                  ["Branches de vida longa, abertas por semanas.",
                   "Tags de versão sem branch de trabalho.",
                   "Trabalhar sem branch principal de referência."],
                  "Reduz merge hell. Exige feature flags e suite de testes confiável. O trunk continua "
                  "sendo a referência compartilhada e os branches continuam existindo — o que muda "
                  "é o tempo de vida deles, medido em horas e não em semanas.",
                  statement_en="Trunk-based development prefers:",
                  correct_en="Short-lived branches and frequent merges into main.",
                  wrong_en=["Long-lived branches, kept open for weeks and months.",
                            "Version tags without a working branch.",
                            "Working without a shared main reference branch."],
                  explanation_en="Reduces merge hell. Requires feature flags and a reliable test suite. The trunk "
                  "stays the shared reference and branches still exist — what changes is their "
                  "lifetime, measured in hours rather than weeks."),
                q("`git stash` faz:",
                  "Salva mudanças locais pendentes para retomar depois.",
                  ["Envia o commit local para o repositório remoto configurado.",
                   "Apaga o commit mais recente do branch atual.",
                   "Cria um branch novo a partir do commit atual."],
                  "Stash empilha. Use `git stash pop` para retomar; aplique a branch correta. Ele mexe "
                  "só na árvore de trabalho e no índice: não cria commit, não cria branch e nada "
                  "sai para o remoto enquanto o stash não vira commit de verdade.",
                  statement_en="`git stash` does:",
                  correct_en="Saves pending local changes so you can resume them later.",
                  wrong_en=["Sends the local commit to the configured remote repository.",
                            "Deletes the most recent commit on the branch.",
                            "Creates a new branch from the current commit."],
                  explanation_en="Stash stacks entries. Use `git stash pop` to resume; apply it to the right "
                  "branch. It touches only the working tree and index: it creates no commit, no "
                  "branch, and nothing reaches the remote until the stash becomes a real commit."),
                q("Conventional Commits é:",
                  "Convenção de mensagens (feat:, fix:, chore:).",
                  ["Um substituto para o Git como ferramenta.",
                   "O hash gerado automaticamente para cada commit.",
                   "Um linter de sintaxe do código-fonte."],
                  "Permite gerar changelog e versionamento automático (semver). É convenção de texto na "
                  "mensagem, verificada por ferramentas como commitlint — não substitui o Git, "
                  "não é o hash do commit e não analisa o código-fonte.",
                  statement_en="Conventional Commits is:",
                  correct_en="A message convention (feat:, fix:, chore:).",
                  wrong_en=["A replacement for Git as a tool.",
                            "The hash generated automatically for each commit.",
                            "A linter for source code syntax."],
                  explanation_en="Enables automatic changelog generation and version bumping (semver). It is a "
                  "text convention in the message, checked by tools like commitlint — it does not "
                  "replace Git, is not the commit hash, and does not analyze source code."),
                q("Em LFS, arquivos grandes:",
                  "Ficam em storage separado, com pointer no repo.",
                  ["Apagam o histórico anterior daquele arquivo.",
                   "Não funcionam em um repositório Git comum.",
                   "Ficam compactados em um arquivo zip anexado ao commit."],
                  "Útil para mídia/binários. Repo principal continua leve; LFS é cobrado por banda. "
                  "O arquivo grande vai para um serviço externo e o commit guarda só o ponteiro: "
                  "nada é zipado no commit e o histórico anterior segue intacto.",
                  statement_en="In LFS, large files:",
                  correct_en="Live in separate storage, with a pointer in the repo.",
                  wrong_en=["Erase the previous history of that file.",
                            "Don't work in a regular Git repository.",
                            "Get compressed into a zip file attached to the commit."],
                  explanation_en="Useful for media/binaries. The main repo stays lightweight; LFS storage is "
                  "billed by bandwidth. The big file goes to an external service and the commit "
                  "keeps only the pointer: nothing is zipped in, and prior history stays intact."),
            ],
        },
        # =====================================================================
...
warning.</li>
            ...
            "questions": [
                q("`terraform plan` faz:",
                  ...
                  ["Aplica a mudança direto no provedor, sem diff.",
                   "Apaga o arquivo de estado armazenado remotamente.",
                   "Cria uma cópia de backup do estado atual do Terraform."],
                  "Plan gera plano determinístico (que apply vai consumir). Ler atentamente evita "
                  "surpresas. Ele é read-only: quem escreve no provedor é `apply`, quem remove o "
                  "estado é `state rm`, e backup do state é responsabilidade do backend.",
                  statement_en="`terraform plan` does:",
                  correct_en="Calculates the diff between current and desired state, without applying it.",
                  wrong_en=["Applies the change directly to the provider, without showing any diff first.",
                            "Deletes the state file stored remotely in the backend.",
                            "Creates a backup copy of Terraform's current state file."],
                  explanation_en="Plan generates a deterministic plan (which apply will consume). Reading it "
                  "carefully avoids surprises. It is read-only: `apply` writes to the provider, "
                  "`state rm` removes state, and backing state up is the backend's job."),
                q("Estado remoto serve para:",
                  "Compartilhar entre membros do time com lock.",
                  ["Substituir a configuração de IAM na conta.",
                   "Habilitar HTTPS nas chamadas à API do provedor.",
                   "Acelerar o plan na máquina local."],
                  "Sem state remoto, devs apagam o trabalho um do outro. Com lock, só um apply roda por "
                  "vez. O backend não mexe em IAM (que continua sendo pré-requisito), não muda o "
                  "transporte HTTPS da API e tende a deixar o plan um pouco mais lento.",
                  statement_en="Remote state is used to:",
                  correct_en="Share state between team members with locking.",
                  wrong_en=["Replace the need to configure IAM on the account.",
                            "Enable HTTPS on calls to the provider API.",
                            "Speed up plan on the local machine."],
                  explanation_en="Without remote state, devs overwrite each other's work. With locking, only one "
                  "apply runs at a time. The backend does not touch IAM (still a prerequisite), does "
                  "not change the API's HTTPS transport, and tends to make plan slightly slower."),
                q("Por que NÃO commitar tfstate?",
                  "Pode conter segredos e gera conflito.",
                  ["Falha o comando git ao tentar versionar.",
                   "Não é permitido pela documentação oficial.",
                   "É grande demais para o GitHub."],
                  "State guarda valores reais (incluindo passwords). Em git público, vira manchete "
                  "instantânea. Tecnicamente o git versiona o arquivo sem reclamar e o tamanho "
                  "raramente é o problema — o risco é vazamento mais conflito de merge no JSON.",
                  statement_en="Why should tfstate NOT be committed?",
                  correct_en="It can contain secrets and generates merge conflicts.",
                  wrong_en=["It makes the git command fail whenever you try to version it.",
                            "It isn't allowed by the official documentation.",
                            "It's too large for GitHub's file size limit."],
                  explanation_en="State stores real values (including passwords). In a public git repo, it "
                  "instantly becomes a headline. Technically git versions the file without "
                  "complaining and size is rarely the issue — the risk is leakage plus JSON merge conflicts."),
                q("Módulo Terraform serve para:",
                  "Encapsular e reutilizar componentes de infra.",
                  ["Criar uma VPN entre duas redes na nuvem.",
                   "Substituir o provider do bloco terraform.",
                   "Gerar log detalhado de cada execução do apply."],
                  "Padroniza configurações da empresa. Versione com tags Git. Módulo é agrupamento de "
                  "recursos parametrizado: não substitui o provider, não cria VPN por si só e "
                  "log de execução vem de TF_LOG, não do módulo.",
                  statement_en="A Terraform module is used to:",
                  correct_en="Encapsulate and reuse infrastructure components.",
                  wrong_en=["Create a VPN between two networks in the cloud.",
                            "Replace the provider set in the terraform block.",
                            "Generate a detailed log of every apply run."],
                  explanation_en="Standardizes the company's configurations. Version it with Git tags. A module "
                  "is a parameterized grouping of resources: it does not replace the provider, does "
                  "not create a VPN by itself, and execution logs come from TF_LOG, not modules."),
                q("`terraform import` serve para:",
                  "Trazer recurso existente para o estado.",
                  ["Renomear um módulo dentro do código.",
                   "Aplicar um plano já gerado pelo plan.",
                   "Apagar um recurso gerenciado pelo estado."],
                  "Útil ao migrar de console-feito para IaC. TF 1.5+ tem `import` block declarativo. "
                  "Renomear endereço é `state mv`, remover do estado é `state rm` e consumir um "
                  "plano salvo é `apply plano.tfplan` — cada tarefa tem seu comando.",
                  statement_en="`terraform import` is used to:",
                  correct_en="Bring an existing resource into the managed state.",
                  wrong_en=["Rename a module that exists in the code.",
                            "Apply a plan already generated by plan.",
                            "Delete a resource that is managed by the current state."],
                  explanation_en="Useful when migrating from console-made resources to IaC. TF 1.5+ has a "
                  "declarative `import` block. Renaming an address is `state mv`, removing from "
                  "state is `state rm`, and consuming a saved plan is `apply plan.tfplan`."),
                q("OpenTofu é:",
                  "Fork open source do Terraform mantido pela Linux Foundation.",
                  ["Outro provider oficial mantido pela HashiCorp.",
                   "Uma DSL diferente, sem relação com o HCL.",
                   "Uma extensão de IDE que destaca a sintaxe HCL dentro do editor."],
                  "Criado após mudança de licença do Terraform para BSL. Compatível com módulos "
                  "existentes. É um binário CLI completo, não um provider nem plugin de editor, e "
                  "continua lendo o mesmo HCL do Terraform.",
                  statement_en="OpenTofu is:",
                  correct_en="An open source fork of Terraform maintained by the Linux Foundation.",
                  wrong_en=["Another official provider maintained by HashiCorp for cloud resources.",
                            "A different DSL, unrelated to HCL.",
                            "An IDE extension that highlights HCL syntax inside the editor."],
                  explanation_en="Created after Terraform's license changed to BSL. Compatible with existing "
                  "modules. It is a full CLI binary, not a provider or an editor plugin, and it "
                  "still reads the same HCL as Terraform."),
                q("Para evitar drift:",
                  "Faça plan/apply periodicamente e proíba mudanças manuais.",
                  ["Apague o arquivo de estado ao perceber divergência.",
                   "Use o console para fazer as mudanças de infraestrutura.",
                   "Edite o tfstate manualmente para corrigir cada divergência encontrada."],
                  "Drift detection em CI noturno é boa prática. Combine com SCPs que bloqueiem mudanças "
                  "manuais. Apagar ou editar o tfstate à mão piora o quadro: o estado deixa de "
                  "descrever a realidade e o próximo apply destrói ou recria recursos.",
                  statement_en="To avoid drift:",
                  correct_en="Run plan/apply periodically and forbid manual changes.",
                  wrong_en=["Delete the state file when you notice a divergence.",
                            "Use the console to make infrastructure changes.",
                            "Edit the tfstate by hand to fix each divergence you find."],
                  explanation_en="Nightly drift detection in CI is good practice. Combine it with SCPs that block "
                  "manual changes. Deleting or hand-editing the tfstate makes it worse: state stops "
                  "describing reality and the next apply destroys or recreates resources."),
                q("Variável sensível em Terraform:",
                  "Marque com sensitive = true.",
                  ["Marque com secret = true.",
                   "Coloque o valor na description.",
                   "Imprima o valor no output para conferência."],
                  "Evita que o valor apareça em outputs/log. Combine com TFC/Vault para evitar plaintext "
                  "no state. O argumento se chama `sensitive`, não `secret`; description e "
                  "comentário ficam em texto puro no repositório e não protegem nada.",
                  statement_en="A sensitive variable in Terraform:",
                  correct_en="Mark it with sensitive = true.",
                  wrong_en=["Mark it with secret = true.",
                            "Put the value in the description.",
                            "Print the value in the output for checking."],
                  explanation_en="Prevents the value from appearing in outputs/logs. Combine with TFC/Vault to "
                  "avoid plaintext in state. The argument is `sensitive`, not `secret`; description "
                  "and comments sit in plain text in the repo and protect nothing."),
                q("Provider é:",
                  "Plugin que conecta Terraform a uma API (AWS, GCP, etc.).",
                  ["Um tipo de variável usado dentro do HCL.",
                   "O hash calculado a partir do plano gerado.",
                   "O componente responsável por guardar o estado remotamente."],
                  "Existem providers oficiais e da comunidade (Cloudflare, GitHub, K8s, Datadog...). "
                  "Provider traduz HCL em chamadas de API; guardar estado é papel do backend e o "
                  "hash do plano é só um identificador do arquivo gerado.",
                  statement_en="A provider is:",
                  correct_en="The plugin that connects Terraform to an API (AWS, GCP, etc.).",
                  wrong_en=["A type of variable used within HCL syntax.",
                            "The hash calculated from the generated plan.",
                            "The component responsible for storing the state remotely in a backend."],
                  explanation_en="There are official and community providers (Cloudflare, GitHub, K8s, "
                  "Datadog...). A provider translates HCL into API calls; storing state is the "
                  "backend's role, and the plan hash is just an identifier of the generated file."),
                q("Lock em backend remoto evita:",
                  "Dois apply simultâneos corrompendo estado.",
                  ["Custo adicional cobrado pelo provedor.",
                   "Backup do estado feito antes de cada apply.",
                   "Importação de um recurso existente para o estado."],
                  "S3+DynamoDB usa item lock; TFC usa lock interno. Sem isso, race condition no state. "
                  "O lock é sobre concorrência: ele não impede import, não faz backup e a "
                  "cobrança do DynamoDB até adiciona um custo pequeno.",
                  statement_en="Locking on a remote backend prevents:",
                  correct_en="Two simultaneous applies corrupting the state.",
                  wrong_en=["Extra cost charged by the cloud provider.",
                            "A state backup made before every apply.",
                            "Importing an already existing resource into the state."],
                  explanation_en="S3+DynamoDB uses item locking; TFC uses an internal lock. Without it, race "
                  "conditions hit the state. The lock is about concurrency: it does not block "
                  "imports, does not back anything up, and DynamoDB even adds a small cost."),
            ],
        },
        # =====================================================================
...
expected.</li>
            ...
            "questions": [
                ...
                q("Idempotência significa:",
                  ...
                  ["Falha ao encontrar um laço de repetição.",
                   "Cria um recurso novo a cada execução.",
                  ...
                  "O oposto seria criar recurso novo a cada execução ou produzir resultado "
                  "imprevisível; laço de repetição é ortogonal e continua permitido.",
                  statement_en="Idempotency means:",
                  correct_en="Running the same task N times yields the same state.",
                  wrong_en=["It fails when it finds a loop.",
                            "It creates a new resource on every run.",
                            "It produces a different result on every run, at random."],
                  explanation_en="It lets you run playbooks confidently on already-configured systems "
                  "(convergence). The opposite would be creating a new resource on every run or "
                  "producing unpredictable results; loops are orthogonal and remain allowed."),
                q("ansible-vault serve para:",
                  "Criptografar arquivos com segredos no repositório.",
                  ["Substituir a necessidade de um KMS externo.",
                   "Comprimir o playbook antes de rodar.",
                   "Compactar o log gerado durante a execução do playbook."],
                  "Bom para projetos pequenos. Em escala, prefira lookups para Vault/Secrets Manager. "
                  "O vault do Ansible cifra arquivos com AES256 no repositório: não comprime nada "
                  "e não substitui um KMS, que ainda guarda e rotaciona a chave mestre.",
                  statement_en="ansible-vault is used to:",
                  correct_en="Encrypt files that hold secrets in the repository.",
                  wrong_en=["Replace the need for an external KMS.",
                            "Shrink the playbook before it runs.",
                ...
                q("Inventário pode ser:",
                  ...
                  ["Restrito ao formato INI, sem YAML.",
                   "Estático, sem inventário dinâmico.",
                   "Dinâmico, gerado por um plugin a cada execução do playbook."],
                  "Dinâmico é essencial em cloud com auto-scaling, onde IPs mudam. Os dois modos "
                  "coexistem: o estático aceita INI e YAML, e o dinâmico vem de script ou plugin "
                  "(aws_ec2, gcp_compute) consultado a cada execução.",
                  statement_en="Inventory can be:",
                  correct_en="Static (file) or dynamic (script/plugin).",
                  wrong_en=["Restricted to INI format, with no YAML.",
                            "Static, with no dynamic inventory.",
                            "Dynamic, generated by a plugin on every playbook run."],
                  explanation_en="Dynamic inventory is essential in cloud with auto-scaling, where IPs change. "
                  "Both modes coexist: static accepts INI and YAML, and dynamic comes from a "
                  "script or plugin (aws_ec2, gcp_compute) queried on every run."),
                q("Role em Ansible é:",
                  "Conjunto reutilizável de tasks, handlers, templates, etc.",
                  ["Um comando de shell rodado num host remoto.",
                   "Um tipo de host dentro do inventário.",
                  ...
                  "Role é unidade de reúso do próprio Ansible — não é um comando isolado, um grupo "
                  "de inventário nem a role de IAM da nuvem, apesar do nome coincidir.",
                  statement_en="An Ansible role is:",
                  correct_en="A reusable set of tasks, handlers, templates, and related files.",
                  wrong_en=["A shell command run on a remote host.",
                            "A host type inside the inventory.",
                            "An IAM access policy applied to a cloud account in the provider."],
                  explanation_en="The standard layout (tasks/, handlers/, defaults/, templates/) makes sharing "
                  "easier. A role is Ansible's own unit of reuse — not a single command, an "
                  "inventory group, or a cloud IAM role, despite the shared name."),
                q("Handlers são executados:",
                  "Apenas quando um task notifica e tem mudança.",
                  ["No início, antes das outras tasks do playbook.",
                   "De forma aleatória, sem relação com a task.",
                   "Quando uma task anterior termina em erro."],
                  "Padrão clássico: copiar nginx.conf → notify 'restart nginx'. Restart só ocorre se "
                  "houve mudança. Handler roda no fim do play, na ordem em que foi declarado, e "
                  "erro em task anterior normalmente aborta o play em vez de disparar handler.",
                  statement_en="Handlers run:",
                  correct_en="Only when a task notifies them and a change occurred.",
                  wrong_en=["At the start, before every other task in the playbook.",
                            "At random, unrelated to the task outcome.",
                            "When an earlier task ends with an explicit error."],
                  explanation_en="Classic pattern: copy nginx.conf → notify 'restart nginx'. Restart only if "
                  "something changed. Handlers run at the end of the play, in declaration order, "
                  "and an earlier failure usually aborts the play instead of firing a handler."),
                ...
                q("ansible-lint serve para:",
                  ...
                  ["Substituir o Ansible como ferramenta.",
                   "Compilar o YAML antes do playbook.",
                   "Publicar a role no Galaxy."],
                  "Pega coisas como 'task sem nome', 'shell sem creates', 'sudo redundante'. É um "
                  "analisador estático que roda ao lado do Ansible: YAML não é compilado, e "
                  "publicar role no Galaxy é trabalho do `ansible-galaxy`.",
                  statement_en="ansible-lint is used to:",
                  correct_en="Detect bad practices in playbooks.",
                  wrong_en=["Replace Ansible itself as a tool.",
                            "Compile the YAML before the playbook.",
                            "Publish the role to Galaxy."],
                  explanation_en="It catches things like 'unnamed task', 'shell without creates', 'redundant "
                  "sudo'. It is a static analyzer that runs alongside Ansible: YAML is not "
                  "compiled, and publishing a role to Galaxy is `ansible-galaxy`'s job."),
                q("Modo `--check` faz:",
                  "Dry-run, simulando sem aplicar.",
                  ["Aplica a mudança assim mesmo.",
                   "Backup do host antes de aplicar.",
                   "Reinicia o agente no host remoto."],
                  "Combine com `--diff` para ver o que mudaria. Útil em PR antes de aplicar. Nada é "
                  "escrito no host: não há backup automático, não existe agente para reiniciar e "
                  "erros continuam sendo reportados normalmente.",
                  statement_en="`--check` mode does:",
                  correct_en="A dry-run, simulating without applying.",
                  wrong_en=["Applies while ignoring errors.",
                            "A host backup before applying.",
                            "Restarts the agent on the remote managed host."],
                  explanation_en="Combine with `--diff` to see what would change. Useful in a PR before applying. "
                  "Nothing is written to the host: there is no automatic backup, no agent to "
                  "restart, and errors are still reported normally."),
...
the same incident.</p>""",
            ...
            "questions": [
                q("Senha em código é:",
                  ...
                  ["Encriptada por padrão em qualquer repositório git privado.",
                   "Uma boa prática recomendada por especialistas.",
                   "Imune a vazamento se o repositório for público."],
                  "Crawlers buscam por padrões como `AKIA...` em segundos após push público. Git guarda "
                  "o arquivo como texto puro, público ou privado: não há cifragem automática, e "
                  "um repositório privado tornado público expõe todo o histórico de uma vez.",
                  statement_en="A password in code is:",
                  correct_en="A critical risk — a public clone is enough to leak it.",
                  wrong_en=["Encrypted by default in any private git repository hosted remotely.",
                            "A widely recommended practice among specialists.",
                            "Immune to leaks if the repo becomes public."],
                  explanation_en="Crawlers search for patterns like `AKIA...` within seconds of a public push. "
                  "Git stores the file as plain text, public or private: there is no automatic "
                  "encryption, and flipping a repo to public exposes the whole history at once."),
                q("Vault dynamic secrets:",
                  "Geram credenciais temporárias por demanda.",
                  ["Substituem a necessidade de TLS na conexão.",
                   "Um registro de DNS para o serviço.",
                   "Um arquivo criptografado no disco local."],
                  "Diminui janela de exposição, e revogação é trivial, basta TTL expirar. O Vault cria a "
                  "credencial no destino (banco, cloud, SSH) na hora do pedido — nada fica em "
                  "arquivo no disco, e o transporte continua precisando de TLS.",
                  statement_en="Vault dynamic secrets:",
                  correct_en="Generate temporary credentials on demand.",
                  wrong_en=["Replace the need for TLS on the connection.",
                            "A DNS record for the service.",
                            "An encrypted file on the local disk."],
                  explanation_en="They shrink the exposure window, and revocation is trivial — just wait for the "
                  "TTL to expire. Vault creates the credential in the target (database, cloud, SSH) "
                  "at request time — nothing sits in a file on disk, and TLS is still required."),
                q("OIDC em CI evita:",
                  "Armazenar chaves longas estáticas.",
                  ["Usar container Docker no pipeline.",
                   "Atualizar dependências antes do deploy.",
                   "Rodar o build em menos tempo."],
                  "GitHub emite token JWT efêmero; AWS valida e devolve credencial STS, sem segredo "
                  "persistente. O ganho é de identidade, não de desempenho: o build não fica mais "
                  "rápido, o container continua igual e dependências seguem sendo atualizadas à parte.",
                  statement_en="OIDC in CI avoids:",
                  correct_en="Storing long-lived static keys.",
                  wrong_en=["Using Docker containers in the pipeline.",
                            "Updating dependencies before deploy.",
                            "Running the build faster."],
                  explanation_en="GitHub issues an ephemeral JWT; AWS validates it and returns STS credentials — "
                  "no persistent secret. The gain is in identity, not performance: the build is not "
                  "faster, the container is unchanged, and dependencies are still updated separately."),
                q("SOPS criptografa:",
                  "Arquivos YAML/JSON com chaves KMS.",
                  ["Senhas em texto puro no arquivo.",
                   "O hash do conteúdo do arquivo.",
                   "Arquivos binários, não texto estruturado."],
                  "Permite commitar arquivo cripto no repo (GitOps friendly). Decryption só com permissão "
                  "KMS. O SOPS cifra valor por valor mantendo as chaves legíveis, então o diff "
                  "continua útil — e hash não serve aqui, porque precisa ser reversível.",
                  statement_en="SOPS encrypts:",
                  correct_en="YAML/JSON files with KMS keys.",
                  wrong_en=["Plaintext passwords in the file.",
                            "A hash of the file contents.",
                            "Binary files, not structured text."],
                  explanation_en="Lets you commit encrypted files to the repo (GitOps-friendly). Decryption "
                  "requires KMS permission. SOPS encrypts value by value, keeping keys readable so "
                  "diffs stay useful — and a hash would not work here, since this must be reversible."),
                q("Rotação automática reduz:",
                  "Janela de exposição se a senha vazar.",
                  ["A latência entre cliente e cofre.",
                   "O custo mensal do serviço de chave.",
                   "O tamanho em disco do arquivo de credencial."],
                  "Mesmo se um atacante captura, a credencial vira inválida em poucos dias. Rotação é "
                  "controle de tempo de vida: não muda latência, tamanho de arquivo nem custo do "
                  "cofre — se muda, é para cima, por causa das chamadas extras.",
                  statement_en="Automatic rotation reduces:",
                  correct_en="The exposure window if the password leaks.",
                  wrong_en=["Latency between client and vault.",
                            "The monthly cost of the key service.",
                            "The disk size of the file holding the credential."],
                  explanation_en="Even if an attacker captures it, the credential becomes invalid within a few "
                  "days. Rotation is a lifetime control: it changes neither latency, file size, nor "
                  "vault cost — and if it moves cost at all, it moves it up, from the extra calls."),
                q("Compartilhar segredo via Slack:",
                  "Risco de exposição persistente, preferir cofres.",
                  ["Auto-expira depois do tempo configurado.",
                   "Boa prática para troca rápida de informação.",
                   "Encriptado por padrão em qualquer canal corporativo."],
                  "Mensagens permanecem em logs corporativos, integrações, exports. Use 1Password share / "
                  "Vault link com TTL. A mensagem não expira sozinha no plano padrão e a cifra em "
                  "trânsito não impede que o admin, o bot e o export leiam o conteúdo depois.",
                  statement_en="Sharing a secret via Slack:",
                  correct_en="Risks persistent exposure — prefer vaults.",
                  wrong_en=["Auto-expires after the configured time.",
                            "A good practice for quick information exchange.",
                            "Encrypted by default in a corporate channel."],
                  explanation_en="Messages linger in corporate logs, integrations, and exports. Use 1Password "
                  "share / Vault link with TTL. The message does not expire on its own by default, "
                  "and transport encryption does not stop admins, bots and exports from reading it."),
                q("`.env.example` deve conter:",
                  "Apenas as chaves esperadas, sem valores reais.",
                  ["Um backup completo do banco de produção da empresa.",
                   "Um token de acesso ao KMS da conta.",
                   "A senha real usada no ambiente de produção."],
                  "Documenta variáveis necessárias, mas valores ficam fora do repo. O arquivo é "
                  "documentação versionada: qualquer valor real ali (senha, token, dump) vaza junto "
                  "com o clone e ainda envelhece mal quando o segredo rotaciona.",
                  ...
                  wrong_en=["A full backup of the company's production database.",
                            "An access token to the account's KMS.",
                            "The real password used in production."],
                  explanation_en="It documents required variables, but values stay out of the repo. The file is "
                  "versioned documentation: any real value in it (password, token, dump) leaks with "
                  "the clone and also goes stale as soon as the secret rotates."),
                q("Pre-commit hook útil:",
                  "Detectar segredos com gitleaks ou trufflehog.",
                  ["Forçar um push direto para o branch principal do repo.",
                   "Apagar o histórico do segredo vazado.",
                   "Comprimir o repositório antes do commit."],
                  "Bloqueia push antes do segredo sair do laptop. Combine com checagem server-side. "
                  "O hook roda antes do commit existir, então ele previne: reescrever histórico "
                  "(`git filter-repo`) é remediação posterior, e compactação é tarefa do `gc`.",
                  statement_en="A useful pre-commit hook:",
                  correct_en="Detect secrets with gitleaks or trufflehog.",
                  wrong_en=["Force a direct push to the main branch of the repo.",
                            "Delete the history of the leaked secret.",
                            "Compress the repository before the commit."],
                  explanation_en="Blocks the push before the secret leaves the laptop. Combine with server-side "
                  "checks. The hook runs before the commit exists, so it prevents: rewriting history "
                  "(`git filter-repo`) is later remediation, and packing is `gc`'s job."),
                q("Em K8s, segredos como Secret são:",
                  "Base64 encoded, NÃO criptografados por padrão.",
                  ["Hash irreversível, sem volta ao valor original.",
                   "Criptografados com chave gerenciada pelo cluster.",
                   "Incapazes de guardar valor binário."],
                  "Habilite encryption-at-rest no etcd e use ferramentas como SealedSecrets/External "
                  "Secrets. Base64 é codificação, não cifra: qualquer um decodifica, o valor volta "
                  "inteiro (por isso não é hash) e binário cabe justamente por causa dela.",
                  statement_en="In K8s, Secret objects are:",
                  correct_en="Base64-encoded, NOT encrypted by default.",
                  wrong_en=["An irreversible hash, with no way back to the value.",
                            "Encrypted with a cluster-managed key.",
                            "Unable to store binary values."],
                  explanation_en="Enable encryption-at-rest on etcd and use tools like SealedSecrets/External "
                  "Secrets. Base64 is encoding, not encryption: anyone can decode it, the value "
                  "comes back whole (so it is not a hash), and binary fits precisely because of it."),
                q("Vault transit engine serve para:",
                  "Criptografia como serviço (encrypt/decrypt) sem expor a chave.",
                  ["Auditoria de permissões do IAM da conta.",
                   "Provisionamento automático de máquina virtual sob demanda na conta.",
                   "Backup periódico do log da aplicação em produção."],
                  "App envia plaintext, recebe ciphertext. Chave nunca sai do Vault. Bom para campos "
                  "sensíveis em DB. O transit não guarda dado nem provisiona recurso: rotação de "
                  "chave é transparente e a aplicação nunca chega a ver o material criptográfico.",
                  statement_en="Vault transit engine is used for:",
                  correct_en="Encryption as a service (encrypt/decrypt) without exposing the key.",
                  wrong_en=["Auditing of IAM permissions on the account.",
                            "Automatic on-demand provisioning of virtual machines in the account.",
                            "Periodic backup of application logs in production."],
                  explanation_en="The app sends plaintext and receives ciphertext. The key never leaves Vault. "
                  "Good for sensitive DB fields. Transit stores no data and provisions nothing: key "
                  "rotation is transparent and the app never sees the cryptographic material."),
            ],
        },
        # =====================================================================
        # 3.5 CI/CD Básico
...
""",
            ...
            "questions": [
                ...
                q("Deploy canário:",
                  ...
                  ["Uma ferramenta de edição de código.",
                   "Roda em ambiente de desenvolvimento.",
                   "Substitui completamente a estratégia blue-green de deploy."],
                  "Origem: canário em mina de carvão. Métricas guiam quando avançar/reverter. É uma "
                  "estratégia de produção, não de ambiente de teste, e convive com blue-green: "
                  "canário mede risco em fatia pequena, blue-green troca o ambiente inteiro.",
                  ...
                  wrong_en=["A code-editing tool, not a deploy strategy.",
                            "Runs in a development environment only.",
                            "Fully replaces the blue-green deploy strategy in production."],
                  explanation_en="Origin: canary in a coal mine. Metrics guide when to advance or roll back. It "
                  "is a production strategy, not a test-environment one, and it coexists with "
                  "blue-green: canary measures risk on a small slice, blue-green swaps the whole environment."),
                q("Pipeline as code é:",
                  "Definir o pipeline em arquivo versionado no repo.",
                  ["Manter o pipeline fora de qualquer controle de versão.",
                   "Rodar o pipeline manualmente a cada vez.",
                   "Escrever o pipeline só em script bash."],
                  "Mudanças no pipeline passam pelo mesmo PR review do código. O que define o padrão é o "
                  "arquivo versionado ao lado da aplicação (.github/workflows, .gitlab-ci.yml) — a "
                  "linguagem pode ser YAML, Groovy ou bash, e o gatilho continua sendo automático.",
                  statement_en="Pipeline as code is:",
                  correct_en="Defining the pipeline in a versioned file in the repo.",
                  wrong_en=["Keeping the pipeline outside any kind of version control system.",
                            "Running the pipeline manually every time.",
                            "Writing the pipeline only as a bash script."],
                  explanation_en="Pipeline changes go through the same PR review as application code. What "
                  "defines the pattern is the versioned file next to the app (.github/workflows, "
                  ".gitlab-ci.yml) — the language may be YAML, Groovy or bash, and triggers stay automatic."),
                q("Falha de teste deve:",
                  "Bloquear o merge/deploy.",
                  ["Ser ignorada pelo pipeline.",
                   "Acelerar o release.",
                   "Gerar só um aviso (warning)."],
                  "Sem bloqueio de verdade o pipeline morre: teste que só emite aviso vira ruído "
                  "ignorado em uma semana, e apressar o release depois de uma falha é como o "
                  "incidente começa. Triagem ágil para flaky tests é essencial.",
                  statement_en="A test failure should:",
                  correct_en="Block the merge/deploy.",
                  wrong_en=["Be ignored by the pipeline.",
                            "Speed up the release.",
                            "Only emit a warning."],
                  explanation_en="Without a hard stop, the pipeline is dead: a test that only warns becomes "
                  "ignored noise within a week, and rushing the release after a failure is exactly "
                  "how incidents start. Agile triage for flaky tests is essential."),
                q("Rollback rápido depende de:",
                  "Artefatos imutáveis e healthchecks.",
                  ["Artefato fora do controle de versão.",
                   "Backup do disco antes do deploy.",
                   "Reconstruir o artefato a cada rollback."],
                  "Re-deploy do artefato anterior leva segundos; rebuild leva minutos. Por isso o "
                  "artefato precisa estar versionado e disponível no registry: backup de disco não "
                  "devolve a versão anterior da aplicação, devolve o estado da máquina.",
                  statement_en="Fast rollback depends on:",
                  correct_en="Immutable artifacts and healthchecks.",
                  wrong_en=["Artifacts outside version control.",
                            "A disk backup before each deploy.",
                            "Rebuilding the artifact on every rollback."],
                  explanation_en="Re-deploying the previous artifact takes seconds; a rebuild takes minutes. That "
                  "is why the artifact must be versioned and available in the registry: a disk "
                  "backup restores machine state, not the previous application version."),
                q("Cache em CI serve para:",
                  "Acelerar builds reaproveitando dependências.",
                  ["Compactar o log gerado durante a execução do pipeline.",
                   "Trocar o runner que executa o pipeline.",
                   "Substituir a necessidade de rodar testes."],
                  "Cuide de invalidação correta (chave por lockfile, não por branch arbitrário). O cache "
                  "só evita baixar e recompilar o que não mudou: ele não escolhe runner, não mexe "
                  "em log e não substitui nenhuma etapa de teste do pipeline.",
                  statement_en="CI cache is used to:",
                  correct_en="Speed up builds by reusing dependencies.",
                  wrong_en=["Compress the logs produced while the pipeline runs.",
                            "Swap the runner that executes the pipeline.",
                            "Replace the need to run tests."],
                  explanation_en="Mind correct invalidation (key by lockfile, not by an arbitrary branch). The "
                  "cache only avoids re-downloading and recompiling what did not change: it picks "
                  "no runner, touches no logs, and replaces no test stage."),
                q("Matrix builds servem para:",
                  "Rodar a mesma pipeline com várias combinações (versões/SO).",
                  ["Reduzir o número de testes do pipeline.",
                   "Aumentar o tamanho do cache do build.",
                   "Trocar a IDE usada pelo desenvolvedor no dia a dia da equipe."],
                  "Ex.: testar Python 3.10/3.11/3.12 × Linux/macOS em paralelo. A matriz multiplica a "
                  "execução em vez de reduzi-la: mais jobs, mais cobertura de combinações e, "
                  "consequentemente, mais tempo total de CPU (mas o mesmo tempo de relógio).",
                  statement_en="Matrix builds are used to:",
                  correct_en="Run the same pipeline across several combinations (versions/OS).",
                  wrong_en=["Reduce the number of tests in the pipeline.",
                            "Increase the cache size used in the build.",
                            "Swap the IDE the developer uses for their daily work in the team."],
                  explanation_en="E.g. test Python 3.10/3.11/3.12 × Linux/macOS in parallel. The matrix "
                  "multiplies runs instead of reducing them: more jobs, more combination coverage "
                  "and therefore more total CPU time (though the same wall-clock time)."),
                q("Trunk-based + CI/CD geralmente exige:",
                  "Feature flags e testes automatizados fortes.",
                  ["Rodar sem etapa de integração contínua.",
                   "Fazer o deploy manualmente a cada versão.",
                   "Manter branch de vida longa, aberta por semanas."],
                  "Sem flags, código incompleto não pode ir para main com segurança. O modelo depende "
                  "justamente do contrário dos distratores: CI rodando em cada push, deploy "
                  "automatizado e branches que vivem horas, não semanas.",
                  statement_en="Trunk-based + CI/CD usually requires:",
                  correct_en="Feature flags and strong automated tests.",
                  wrong_en=["Running with no continuous integration stage.",
                            "Deploying manually for every version.",
                            "Keeping long-lived branches open for weeks."],
                  explanation_en="Without flags, incomplete code cannot safely land on main. The model depends on "
                  "the opposite of the distractors: CI running on every push, automated deploys, "
                  "and branches that live hours rather than weeks."),
                q("Artefato imutável significa:",
                  "Mesma versão (hash) é sempre a mesma, usado em todos os ambientes.",
                  ["Pode ser editado depois de publicado, sem gerar hash novo.",
                   "Existe só localmente, na máquina que fez o build.",
                   "Tem um TTL definido, expirando automaticamente depois de um tempo configurado."],
                  "Tag SHA + assinatura (Cosign) garante. 'latest' móvel é o oposto. Imutável quer dizer "
                  "que o conteúdo daquele digest não muda nunca: publicar algo diferente exige um "
                  "hash novo, e o artefato vive no registry, não só na máquina do build.",
                  ...
                  wrong_en=["It can be edited after publish without producing a new hash.",
                            "It exists only on the machine that did the build.",
                            "It has a defined TTL and expires automatically after some configured time window."],
                  explanation_en="SHA tags + signing (Cosign) guarantee it. Movable 'latest' is the opposite. "
                  "Immutable means the content behind that digest never changes: publishing "
                  "something different needs a new hash, and the artifact lives in the registry."),
                q("Argo CD aplica padrão:",
                  "GitOps, repo Git é a fonte da verdade.",
                  ["Transferência de arquivo via FTP.",
                   "Aplicação manual direto no cluster.",
                   "Tarefa agendada (cron) rodando periodicamente."],
                  "Argo observa o repo; quando muda, reconcilia o cluster com o declarado. O gatilho é o "
                  "estado divergente, não um cron nem um humano com kubectl na mão — e o "
                  "transporte é a API do Kubernetes, não cópia de arquivo.",
                  statement_en="Argo CD applies the pattern:",
                  correct_en="GitOps — the Git repo is the source of truth.",
                  wrong_en=["Traditional FTP file transfer.",
                            "Manual application directly on the cluster.",
                            "A cron job running periodically on the cluster."],
                  explanation_en="Argo watches the repo; when it changes, it reconciles the cluster to the "
                  "declared state. The trigger is divergent state, not a cron job or a human with "
                  "kubectl — and the transport is the Kubernetes API, not a file copy."),
            ],
        },
        # =====================================================================
        # 3.6 Linting
...
not just style.</li>
            ...
            "questions": [
                ...
                q("pre-commit serve para:",
                  ...
                  ["Apagar branch no remoto.",
                   "Substituir o CI do repositório.",
                   "Compactar o log do commit."],
...
known today.</li>
            ...
            "questions": [
                q("SAST acrônimo significa:",
                  ...
                  ["System Audit Software Tool.",
                   "Single Access Static Token.",
                ...
                q("Bandit detecta:",
                  ...
                  ["Erros de schema em YAML.",
                   "Vulnerabilidades em código Java.",
                ...
                q("CodeQL roda:",
                  ...
                  ["Só expressões regulares no fonte.",
                   "Queries dentro de um servidor SQL.",
                   "Só no ambiente de produção."],
...
about which level each artifact actually reached.</p>""",
            ...
            "questions": [
                q("SCA significa:",
                  ...
                  ["System Check Authority.",
                   "Secure Code Algorithm.",
                   "Static Code Audit."],
                ...
                q("Lockfile (poetry.lock, package-lock):",
                  ...
                  ["Remove dependências do projeto.",
                   "Define a política de IAM do deploy.",
                   "Ignora as versões declaradas."],
                ...
                q("Política de patching deve definir:",
                  ...
                  ["Só o time responsável.",
                   "Só o custo estimado da correção.",
                   "Só as regras de lint do repo."],
...
tends to create misunderstandings that only get worse in writing.</li>
            ...
            "questions": [
                ...
                q("Checklist em PR ajuda a:",
                  ...
                  ["Aprovar o PR sem rodar testes.",
                   "Tornar a aprovação mais lenta.",
                   "Reduzir a cobertura de testes."],
                ...
                q("Reviewer perceber dado sensível em log:",
                  ...
                  ["Aprovar e corrigir depois.",
                   "Ignorar: log é pouco lido.",
                   "Adicionar mais dado ao log."],
                ...
                q("Métrica saudável:",
                  ...
                  ["PR gigante parado há semanas.",
                   "Volume alto de PR aprovado sem leitura.",
                   "Revisão centralizada no CTO."],
...
infrastructure.</p>""",
            ...
            "questions": [
                ...
                q("Cosign serve para:",
                  ...
                  ["Comprimir a imagem antes do push.",
                   "Substituir o registry do pipeline.",
                   "Compilar o projeto Java no build."],
